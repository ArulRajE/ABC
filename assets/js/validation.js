...
function numbersOnly12(evt) {
    ...
    if ((charCode >= 48 && charCode <= 57) || charCode == 8 || charCode == 9 || charCode == 13 || charCode == 37 || charCode == 39) {
        return (true);
    };
    return (false);
}
            ...
            'columnDefs': [
                ...
                ,
                {
                    ...
                    render: function (dataaa, type, row, meta) {
                        ...
                        // console.log(row);
                        if (type === 'display') {
                            data = '<a href="Alldocuments/' + row[3] + '/' + encodeURIComponent(row[4]) + '" target="_blank"><i class="fas fa-file-alt fa-2x mb-2" data-toggle="tooltip" data-placement="top" style="color:#4667cc;" title="" data-original-title=""></i></a>';
                        }
                        return data;
                    }
...
//           render: function (dataa, type, row, meta) {
//             if (type === 'display') {
//               data = '<a href="Alldocuments/' + stids + '/' + encodeURIComponent(dataa) + '" target="_blank"><i class="fas fa-file-alt fa-2x mb-2" data-toggle="tooltip" data-placement="top" style="color:#4667cc;" title="" data-original-title=""></i></a>';
//             }
//             return data;
//           }
//         },
...
//                 var deleteButton = '';
//                 if (row[13] == 0) {
//                     deleteButton = '<a href="javascript:void(0);" class="btn btn-danger delete-btn" onclick="deletependoc(' + row[1] + ', \'deletetablerowpen\')"><i class="fa fa-trash"></i></a>';
//                   } else {
//                     deleteButton = '<a href="javascript:void(0);" class="btn btn-danger delete-btn" disabled style="cursor: not-allowed;"><i class="fa fa-trash" style="cursor: not-allowed;"></i></a>';
//                   }
//                   return deleteButton;
//             }  
...
// $('#alreadydoc-datatable-list').on('change', 'input[type="checkbox"]', function() {
//     var checkbox = $(this);
//     var checked = checkbox.prop('checked');
//     var checkboxes = $('#alreadydoc-datatable-list input[type="checkbox"]').not(checkbox);
...
//SR No. 9 By sahana 
// //modified by sahana for reuse of document || modified by sahana JC_22
async function getdoc_list(docids, stids) {
    $('#con-link-document').modal('show');
    ...
    try {
        var response = await $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: { formname: "sessiondetails" },
        });
        var sessiondetails;
        try {
            sessiondetails = JSON.parse(response);
        } catch (parseError) {
        ...
        var tablejig = $('#alreadydoc-datatable-list').DataTable({
            ...
            'columnDefs': [
                ...
                //modified by yogesh and sahana to delete pending link document  
                {
                    ...
                    'render': function (data, type, row, meta) {
                        var deleteButton = '';
                        if (row[13] == 0 && row[7] === sessiondetails.login_email) {
                            deleteButton = '<a href="javascript:void(0);" class="btn btn-danger delete-btn" onclick="deletependoc(' + row[1] + ', \'deletetablerowpen\')"><i class="fa fa-trash"></i></a>';
                        } else {
                            deleteButton = '<a href="javascript:void(0);" class="btn btn-danger delete-btn" disabled style="cursor: not-allowed;"><i class="fa fa-trash" style="cursor: not-allowed;"></i></a>';
                        }
...
function gettext_data(val, i) {
    ...
    if (($('#comefromcheck').val() == 'State' || $('#comefromcheck').val() == 'Village / Town') && $('#clickpopup').val() == 'Rename') {
        ...
        if (status == true) {
            ...
            if (i == '') {
                ...
                if ($('#comefromcheck').val() == 'Village / Town') {
                    if ($('#vlevel_' + i + '').val() == $('#vStatus2021_' + i + '').val()) {
                        $('#assignbtn').attr('disabled', true);
                    }
                    else {
                        $('#assignbtn').attr('disabled', false);
                    ...
                    if ($('#toStatus_' + i + '').val() == $('#Statusyear_' + i + '').val()) {
                        $('#assignbtn').attr('disabled', true);
                    }
                    else {
                        $('#assignbtn').attr('disabled', false);
...
function get_doctype_data(data, doccome) {
    ...
    if (data.value != "" && data.value == "Others" && doccome == '') {
        ...
        if (data.value != "" && (data.value == 'Resolution' || data.value == 'Clarification' || data.value == 'Collector Letter' || data.value == 'Others') && doccome == 'comefromdocadd') {
...
function selecteddocumentrow() {
    var rows_selected = $('#alreadydoc-datatable-list').DataTable().column(0).checkboxes.selected();
    // console.log(rows_selected[0]);
    // return false;
    if (rows_selected.length >= 1) {
        $('#alreadydoc-datatable-list').modal('hide');
        // redirectcompleted(rows_selected[0]);
        window.location.href = "adddocument?idsdoc=" + rows_selected[0] + "&come=comefromdocreuse";
    }
    else {
...
function reportfilterst(valdata, come, flag) {
    //modified by gowthami issue related to concordance dropdown refreshing and repeatiing of option

    if ($('#STID').val() != '') {
        if ($('#STID').val() != '' && $('#DTID').val() == '' && $('#SDID').val() == '') {
            if (flag == 'ST') {
...

                {
                    ...
                    render: function (dataa, type, row, meta) {
                        ...
                        if (type === 'display') {
                            ...
                            data = '<a href="javascript:void(0)" onclick="return get_poplistof(\'' + row + '\',\'' + flag + '\');"><i class="mdi mdi-comment-eye fa-2x mb-2" style="color:#4667cc;" data-toggle="tooltip"  data-placement="top" title="" data-original-title="View Remarks"></i></a>';
                        }
                        return data;
                    }
...
function get_users_dist_select_data(data) {
    $.ajax({
        ...
        url: "insert_data.php",
        data: "formname=getdistlistusers&STID2021=" + data.value + "&adminids=" + $('#update_ids').val()
    }).done(function (result) {
...
function get_fromvalue1(value, i) {
    ...
    if (value == '') {
        ...
        $('#fstatus' + i + '').val('').trigger("change"); // Arun
    }
    // JC_35 srikanth
    ...
    if (value != '' && $('#clickpopup').val() == 'Create' && ($('#comefromcheck').val() == 'State' || $('#comefromcheck').val() == 'District')) {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=get_state_status&comefrom=" + $('#comefromcheck').val() + "&clickpopup=" + $('#clickpopup').val() + "&fstids=" + value + "&i=" + i
        }).done(function (result) {
    ...
    else if (value != '' && ($('#clickpopup').val() == 'Create' || $('#clickpopup').val() == 'Reshuffle') && $('#comefromcheck').val() == 'Village / Town') {


        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=get_vt_status&comefrom=" + $('#comefromcheck').val() + "&clickpopup=" + $('#clickpopup').val() + "&fstids=" + value + "&i=" + i
        }).done(function (result) {
    ...
    else if (value != '' && $('#clickpopup').val() == 'Merge' && $('#comefromcheck').val() == 'State') {
    ...
    else if ($('#clickpopup').val() == 'Merge' && $('#comefromcheck').val() == 'Sub-District') {
        ...
        // JC_bheema
        if ($('#comefromcheck').val() == 'Sub-District' && $('#clickpopup').val() == 'Merge' && value != '') {
            $("#action" + i).val('').trigger('change');
        }
        if ($('#comefromcheck').val() == 'Sub-District' && $('#clickpopup').val() == 'Merge' && value == '') {
            $("#action" + i).val('').trigger('change');
    ...
    else if ($('#clickpopup').val() == 'Merge' && $('#comefromcheck').val() == 'Village / Town') {
    ...
    // }
        ...
        // Ends...
        // JC_bheema
        if ($('#comefromcheck').val() == 'District' && $('#clickpopup').val() == 'Merge' && value != '') {
            $("#action" + i).val('').trigger('change');
        }
    }
    ...
    // JC_11 Arul
        ...
        // JC_bheema
        if ($('#comefromcheck').val() == 'Sub-District' && $('#clickpopup').val() == 'Create' && value != '') {
            $("#action" + i).val('').trigger('change');
        }
        if ($('#comefromcheck').val() == 'Sub-District' && $('#clickpopup').val() == 'Create' && value == '') {
            $("#action" + i).val('').trigger('change');
...
function get_to_data(value, i) {
    ...
    if (value.value != '' && $('#clickpopup').val() == 'Merge' && $('#comefromcheck').val() == 'State') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=get_state_status&comefrom=" + $('#comefromcheck').val() + "&clickpopup=" + $('#clickpopup').val() + "&fstids=" + value.value + "&i=" + i
        }).done(function (result) {
    ...
    else if (value.value != '' && $('#clickpopup').val() == 'Rename' && $('#comefromcheck').val() == 'State') {
            ...
            if (finalresult[0] == 'getdata') {
                if (finalresult[3] != '') {
                    ...
                    if (i == '') {
                        ...
                        $('#Status2021_' + i + '').val(finalresult[3]).trigger('change');
    ...
    else if (value.value != '' && ($('#clickpopup').val() == 'Rename' || $('#clickpopup').val() == 'Merge' || $('#clickpopup').val() == 'Partiallysm') && $('#comefromcheck').val() == 'Village / Town') {


        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=get_vt_status&comefrom=" + $('#comefromcheck').val() + "&clickpopup=" + $('#clickpopup').val() + "&fstids=" + value.value + "&i=" + i
        }).done(function (result) {
            ...
            if (finalresult[0] == 'getdata') {
                ...
                if (i == '')
                // $('#newnamecheck').val('');//jc_35 code added by srikanth
                {
                    i = 1;
                }


                $('#vStatus2021_' + i + '').val(finalresult[3]).trigger('change');
                $('#vlevel_' + i + '').val(finalresult[3]);
                if (finalresult[4] == '') {
                    $('#vstatus' + i + '').val('RV').trigger('change');
                    $('#ovstatus_' + i + '').val('RV');
                }
                else if (finalresult[4] != '') {
                    //  alert(finalresult[3]);
                    // alert(finalresult[4]);
                    $('#vstatus' + i + '').val(finalresult[4]).trigger('change');
        ...
        // JC_35 srikanth
        if ($('#clickpopup').val() == 'Rename' || $('#clickpopup').val() == 'Merge' && $('#comefromcheck').val() == 'Village / Town') {
    ...
    //jc-35 refresh
    else if (value.value == '' && ($('#clickpopup').val() == 'Rename' || $('#clickpopup').val() == 'Merge') && $('#comefromcheck').val() == 'District') {
        $('#newnamecheck' + i + '').val('');
        // JC_11 Modified by Arul for disable add M/PM
        if (namefrom.length > 0 && action.length > 0 && statenew.length > 0 && stateid == '' && $('#clickpopup').val() == 'Merge') {
            EnableRow(rn);
        }
        // Ends... 
    ...
    else if (value.value == '' && ($('#clickpopup').val() == 'Rename' || $('#clickpopup').val() == 'Merge') && $('#comefromcheck').val() == 'Sub-District') {
...
function get_fromvalue(value, comedata) {
    if (value != '') {
        ...
        if (fromdata != '') {
            ...
            $("#selected_comemrg").append($('<option>', {
                value: '',
...
function get_sub_district_popup_list(data) {
    ...
    var fdtids = $('select[name="districtgetsub[]"] option:selected').map(function () {
        return this.value;

    }).get();
    var fsdids = $('select[name="subdistrictgetsub[]"] option:selected').map(function () {
        return this.value;
    ...
    if (data.value != '') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopupsub_vtlist&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids
        }).done(function (result) {
...
function get_district_popup_sublist(data) {
    ...
    if (data.value != '') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopupsub_sublist&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids
        }).done(function (result) {
            ...
            if (seleted == 'State') {
                $("#maintitle").html("Merge / Partially Merge - " + finalresult[2] + " / UT");

                $("#comespan").html("[Merge / Partially Merge - " + finalresult[2] + " / UT]");
            ...
            if (finalresult[1] == 'Village / Town') {
                ...
                $("#subdistrictgetsub").append($('<option>', {
                    value: '',
...
function get_district_popup_list(data) {
    ...
    if (data.value != '') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopupsub_list&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids
        }).done(function (result) {
            ...
            if (finalresult[1] == 'Sub-District' || finalresult[1] == 'Village / Town') {
...
function get_district_popup(data, clickpopup) {
    ...
    $.ajax({
        type: "POST",
        url: "insert_data.php",
        data: "formname=getdataofpopupsub&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids
    }).done(function (result) {
        ...
        //delete code added 
            ...
            if (seleted == 'State') {
                ...
                $("#comespan").html("[Merge / Partially Merge - " + seleted + "  / UT]");
                $("#addlable").html("Select " + seleted + " / UT");
                $("#addlable1").html("Select " + seleted + " / UT");
                $("#name2021").attr("placeholder", "" + seleted + " / UT Name");
            }
        ...
        if (seleted == 'Sub-District') {
            ...
            if (finalresult[3] == 'Create' || finalresult[3] == 'Merge' || finalresult[3] == 'Partiallysm' || finalresult[3] == 'Reshuffle') {
                ...
                if (mul[1] == 'undefined' || mul[1] == undefined) {
                    ...
                    $("#action1,#actiona2").children().remove();
                    $("#action1,#actiona2").append($('<option>', {
                        value: '',
                        text: 'Select Action',
                    }));
                    ...
                    // JC_38 Modified By Arul For Partially Split / Merge
                    if ($('#partiallyids').val() != '') {
                    ...
                    //   console.log(mul[1]);
                    $("#id2021" + mul[1] + "").children().remove();

                    $("#id2021" + mul[1] + "").append($('<option>', {
                        value: '',
                        text: 'Select ' + seleted + '',
                    }));
                    ...
                    $("#id2021" + mul[1] + "").val('').trigger('change');


                    $("#action" + mul[1] + "").children().remove();
                    $("#action" + mul[1] + "").append($('<option>', {
                        value: '',
                        text: 'Select Action',
                    }));
                ...
                if (finalresult[3] == 'Reshuffle' && statenew != '' && data.value != '') {
                    $("#statenew1").val(statenew).trigger('change');
                }
                //ends here
            ...
            if (seleted != 'State') {
                ...
                if ((finalresult[3] == 'Create' || finalresult[3] == 'Merge') && finalresult[2] == 'District') {
                    ...
                    if (mul[1] == 'undefined' || mul[1] == undefined) {
                        ...
                        $('select').select2();
                        // Modified By Arun For Validation
                        if ($('#sddistrictget_1').siblings('ul.parsley-errors-list').find('li').length > 0) {
                            $('#selected_come').parsley().validate();
                        }
                        // Ends...
                        $(".AC").css({ 'margin-left': '' });

                        $("#action1").children().remove();
                        $("#action1").append($('<option>', {
                            value: '',
                            text: 'Select Action',
                        }));
                        ...
                        $("#sddistrictget_" + mul[1] + "").children().remove();
                        $("#sddistrictget_" + mul[1] + "").append($('<option>', {
                            value: '',
                            text: 'Select Sub-District',
                        }));
                        if (data.value != '') {
                            ...
                            $('#sddistrictget_' + mul[1] + '').val('').trigger('change');
                        }
                        $('#did2021' + finalresult[3] + '').html('');
                        $('#did2021' + mul[1] + '').html('<select class="form-select selected_come" required name = "namefrom[]" id="id2021' + mul[1] + '" onchange="return get_fromvalue1(this.value,' + mul[1] + ')" ><option value="">Select Village / Town</option></select>');
                        $('select').select2();

                        // Modified By Arun For Validation
                        if ($('#sddistrictget_' + mul[1]).siblings('ul.parsley-errors-list').find('li').length > 0) {
                            $('#id2021' + mul[1]).parsley().validate();
                        }
                        // Ends...
...
function get_sub_district_popup_new(data, clickpopup, i) {
    ...
    // Ends
    // JC_38 Modified By Arul For Partially Split / Merge
    var partiallyids = $('#partiallyids').val();
    // Ends...
    ...
    if ($("#sddistrictget_" + i + "").val() != '') {
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopupsubvillagenew&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids + "&fvtids=" + fvtids + "&i=" + i + "&datavalue=" + data.value + "&jigs=" + jigs
        }).done(function (result) {
            ...
            if (clickpopup == 'Create') {
                ...
                if (seleted == 'State') {
                    $("#maintitle").html("Partially Split & Merge - " + finalresult[2] + " / UT");

                    $("#comespan").html("[Partially Split & Merge - " + finalresult[2] + " / UT]");
                    $("#addlable").html("Enter " + finalresult[2] + " / UT");
                    $("#addlable1").html("Select " + finalresult[2] + " / UT");
                    $("#name2021").attr("placeholder", "" + finalresult[2] + " / UT Name");
                }
            ...
            if (finalresult[2] == 'Village / Town') {
                ...
                //code changed by bheema
                {
                    ...
                    //this value required - Arun
                    $('#selected_come').on('change', function () {
                        if ($(this).val()) {
                            $(this).parsley().removeError('required', { updateClass: true });
                        }
                    });
                ...
                if (i != 1) {
                    $("#id2021" + i + "").children().remove();
                    $("#id2021" + i + "").append($('<option>', {
                        value: '',
            ...
            $("#action" + i + "").children().remove();
            $("#action" + i + "").append($('<option>', {
                value: '',
                text: 'Select Action',
            }));
            $(JSON.parse(finalresult[1])).each(function () {
                if ($('#flagof').val() == 'true') {
                    //  alert('innnnn');
                    if (clickpopup == 'Create' && seleted == 'Village / Town' && this.forreaddetails == 'Full Merge') {
            ...
            // JC_35 srikanth
            else if (clickpopup == 'Merge' && data.value != '' && seleted == 'Village / Town') {

                $('#action' + i + '').val('').trigger('change');
            }

            else if (clickpopup == 'Create' && data.value != '' && seleted == 'Village / Town') {
                if (partiallyids != '') {
                    var jsonresult = JSON.parse(finalresult[1]);
                    var id = jsonresult[0]['forreaddetails'];
                    $('#action' + i + '').val(id).trigger('change');
                } else {
                    $('#action' + i + '').val('').trigger('change');
                }
            }
            //
            // JC_38 Modified By Arul For Partially Split / Merge
            else if (clickpopup == 'Partiallysm' && seleted == 'Village / Town' && partiallyids != '') {
                var jsonresult = JSON.parse(finalresult[1]);
                var id = jsonresult[0]['forreaddetails'];
                $('#action' + i + '').val(id).trigger('change');
            }
        ...
        if (i == 1) {
            ...
            // JIGARGO
            $('#did2021' + i + '').html('');
            $('#did2021' + i + '').html('<select class="form-select selected_come"  ' + req + ' name = "namefrom[]" id="id2021' + i + '" onchange="return get_fromvalue1(this.value,' + i + ')" ><option value="">Select Sub-District</option></select>');
            $('select').select2();
...
function get_sub_district_popup_new_______(data, clickpopup, i) {
    ...
    if ($("#sddistrictget_" + i + "").val() != '') {
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopupsubvillagenew&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids + "&fvtids=" + fvtids + "&i=" + i + "&datavalue=" + data.value
        }).done(function (result) {
...
function getstatus(data, i) {
    ...
    else if (clickpopup == 'Deletion')   /// deleteion added
    {
        // alert($('#ovstatus_'+i+'').val());
        //    
        if (data.value != '') {
...
function get_sub(data, i) {
    ...
    $.ajax({
        type: "POST",
        url: "insert_data.php",
        data: "formname=get_sub&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&data=" + data.value + "&i=" + i + "&vtids=" + vtids
    }).done(function (result) {

        var finalresult = result.split("|");

        $("#vstatus" + i + "").children().remove();

        $("#vstatus" + i + "").append($('<option>', {
            value: '',
            text: 'Select Status',
        }));
        ...
        if ($('#vStatus2021_' + i + '').val() != '' && $('#vStatus2021_' + i + '').val() == 'VILLAGE') {
            var jj = '';
            if ($('#ovstatus_' + i + '').val() != '') {
                jj = $('#ovstatus_' + i + '').val();
            }
        ...
        else if ($('#vStatus2021_' + i + '').val() != '' && $('#vStatus2021_' + i + '').val() == 'TOWN') {
            var jj = '';
            if ($('#ovstatus_' + i + '').val() != '') {
                jj = $('#ovstatus_' + i + '').val();
            }
...
function getvtlist(data, i) {
    ...
    if (clickpopup == 'Merge' || clickpopup == 'Partiallysm' || clickpopup == 'Rename' || clickpopup == 'Deletion' || clickpopup == 'Reshuffle') {
        ...
        if ($("#sddistrictnew" + i + "").val() != '') {
            $.ajax({
                type: "POST",
                url: "insert_data.php",
                data: "formname=getdataofpopupsubvillagenew_to&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids + "&fvtids=" + fvtids
            }).done(function (result) {
                ...
                if (clickpopup == 'Rename') {
                    ...
                    for (var i = 0; i < resac.length; i++) {
                        if (resac[i] == 1) {
                            ...
                            $('select[name="namefrom' + resac[i] + '[]"] option:selected').map(function () {
                                // return this.value;
                                fromdata.push(this.value);
                            });
    ...
    // Action 2021 trigger Arul
    else if (data.value == '' && clickpopup == 'Create' && seleted == "Village / Town") {
        $('#name2021').val('').trigger('keyup');
    }
    else {
        if (clickpopup == 'Addition') {
...
function getvtlist_more(data, j) {
    ...
    if (clickpopup == 'Merge' || clickpopup == 'Partiallysm' || clickpopup == 'Rename'|| clickpopup == 'Deletion') { // jc_07
        ...
        if ($("#sddistrictnew" + j + "").val() != '') {
            $.ajax({
                type: "POST",
                url: "insert_data.php",
                data: "formname=getdataofpopupsubvillagenew_to_more&comefrom=" + seleted + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids + "&fvtids=" + fvtids
            }).done(function (result) {

                var finalresult = result.split("|");

                if (clickpopup == 'Rename'|| clickpopup == 'Deletion') { // JC_07
                    var fromdata = $('select[name="newnamem[]"] option:selected').map(function () {
                        return this.value;
                    ...
                    $("#named2021" + j + "").children().remove();

                    $("#named2021" + j + "").append($('<option>', {
                        value: '',
                        text: 'Select ' + $('#comefromcheck').val() + '',
                    }));
                    ...
                    for (var i = 0; i < fromdata1.length; i++) {
                        ...
                        $("#named2021" + j + " option[value=" + fromdata1[i] + "]").remove();
                    }
                }
                else {
                    var fromdata = $('select[name="namefrom[]"] option:selected').map(function () {
        ...
        // JC_35 srikanth
        if (data.value == '' && clickpopup == 'Rename' && seleted == "Village / Town") {
            $('#named2021' + j + '').val('').trigger('change');
        }
        // 
    ...
    // jc_35 srikanth  
    else if (data.value === '' && clickpopup == 'Addition' && seleted == "Village / Town") {

        $('#name2021' + j + '').val('').trigger('change');
    }
    else if (data.value != '' && clickpopup == 'Addition' && seleted == "Village / Town") {
...
function get_sub_district_popup(data, clickpopup) {
    ...
    $.ajax({
        type: "POST",
        url: "insert_data.php",
        data: "formname=getdataofpopupsubvillage&comefrom=" + seleted + "&DTID=" + districtget + "&STID=" + dids + "&SDID=" + data.value + "&clickpopup=" + clickpopup
    }).done(function (result) {
        ...
        if (finalresult[2] == 'Village / Town') {
            ...
            if (clickpopup == 'Create') {
                ...
                $("#actionmrg1,#actiona2").append($('<option>', {
                    value: '',
                    text: 'Select Action',
                }));
                $(JSON.parse(finalresult[1])).each(function () {
...
function get_village_popup(data) {
    ...
    $.ajax({
        type: "POST",
        url: "insert_data.php",
        data: "formname=getdataofpopupsubward&comefrom=" + seleted + "&DTID=" + districtget + "&STID=" + dids + "&SDID=" + subdistrictget + "&VTID=" + data.value
    }).done(function (result) {
...
function get_district_popup_distdata_add(data, clickpopup, no) {
    ...
    if ((clickpopup == 'Create' || clickpopup == 'Merge' || clickpopup == 'Reshuffle') && data.value != '') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopup_add&comefrom=" + seleted + "&selectstid=" + data.value + "&clickpopup=" + clickpopup + "&no=" + no
        }).done(function (result) {
            ...
            //    console.log(finalresult);
            if (finalresult[1] == 'Sub-District' || finalresult[1] == 'Village / Town') {
                $("#districtget_" + finalresult[3] + "").children().remove();
                $("#districtget_" + finalresult[3] + "").append($('<option>', {
                    value: '',
                    text: 'Select District',
                }));
                ...
                $("#districtget_" + finalresult[3] + "").val('').trigger('change');
                if (finalresult[1] == 'Sub-District') {
                    $('#did2021' + finalresult[3] + '').html('');
                    $('#did2021' + finalresult[3] + '').html('<select class="form-select selected_come" required name = "namefrom[]" id="id2021' + finalresult[3] + '" onchange="return get_fromvalue1(this.value,' + finalresult[3] + ')" ><option value="">Select Sub-District</option></select>');
                    $('select').select2();
                    $('.ACNN' + finalresult[3] + '').css({ 'margin-left': '' });
                }
                else {
                    $('#did2021' + finalresult[3] + '').html('');
                    $('#did2021' + finalresult[3] + '').html('<select class="form-select selected_come" required name = "namefrom[]" id="id2021' + finalresult[3] + '" onchange="return get_fromvalue1(this.value,' + finalresult[3] + ')" ><option value="">Select Village / Town</option></select>');
                    $('select').select2();
                    $('.ACNN' + finalresult[3] + '').css({ 'margin-left': '' });
                }
                ...
                $("#id2021" + finalresult[3] + "").children().remove();
                ...
                $(JSON.parse(finalresult[0])).each(function () {
                    if ($.inArray(this.id, filtered) == -1 && finalresult[1] == 'District') {
                        $("#id2021" + finalresult[3] + "").append($('<option>', {
                            value: this.id,
                            text: this.Name,
                        }));
...
function get_district_popupto_ii(data, clickpopup, i) {
    ...
    if ((clickpopup == 'Create' || clickpopup == 'Addition' || clickpopup == 'Rename'|| clickpopup == 'Deletion') && data.value != '') { // jc_07
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopup_to&comefrom=" + seleted + "&selectstid=" + data.value + "&clickpopup=" + clickpopup
        }).done(function (result) {
            ...
            if (finalresult[1] == 'Sub-District') {
                $("#districtnew" + i + "").children().remove();
                $("#districtnew" + i + "").append($('<option>', {
                    value: '',
                    text: 'Select District',
                }));

                var arr = $('select[name="districtnew[]"] option:selected').map(function () {
                    return this.value;  // $(this).val()
                }).get();
                ...
                if (finalresult[1] != 'District') {
                    ...
                    if (clickpopup == 'Rename') {
                        ...
                        $("#named2021" + i + "").children().remove();

                        $("#named2021" + i + "").append($('<option>', {
                            value: '',
                            text: 'Select ' + $('#comefromcheck').val() + '',
                        }));
                        ...
                        for (var j = 0; j < fromdata1.length; j++) {
                            ...
                            $("#named2021" + i + " option[value=" + fromdata1[j] + "]").remove();
                        }
                    }
                    else {
                        $("#named2021" + i + "").children().remove();
...
function get_district_popupto(data, clickpopup, i) {
    ...
    if ((clickpopup == 'Create' || clickpopup == 'Merge' || clickpopup == 'Partiallysm' || clickpopup == 'Addition' || clickpopup == 'Rename' || clickpopup == 'Deletion' || clickpopup == 'Reshuffle') && data.value != '') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopup_to&comefrom=" + seleted + "&selectstid=" + data.value + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids
        }).done(function (result) {
            ...
            if (finalresult[1] == 'Sub-District') {
                ...
                if (clickpopup == 'Reshuffle') {
                    var fromdata = $('select[name="districtget[]"] option:selected').map(function () {
                        return this.value;
    ...
    // Action 2021 trigger Arul
    else if (data.value == '' && clickpopup == 'Create' && seleted == 'District') {
        $('#name2021').val('').trigger('keyup');
    }
    // Ends...
    // JC_104 Modified by Arul for district refresh in M/PM
...
function get_sddistrict_popupto_ii(data, clickpopup, i) {
    ...
    if ((clickpopup == 'Create' || clickpopup == 'Addition' || clickpopup == 'Rename' || clickpopup == 'Deletion') && data.value != '') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopup_to_sd&comefrom=" + seleted + "&selectdtid=" + data.value + "&clickpopup=" + clickpopup
        }).done(function (result) {
            ...
            if (finalresult[1] == 'Village / Town') {
                ...
                $("#sddistrictnew" + i + "").children().remove();
                $("#sddistrictnew" + i + "").append($('<option>', {
                    value: '',
                    text: 'Select Sub-District',
                }));
            ...
            //srikanth jc_35
            if ((clickpopup == 'Addition') && data.value !== '' && seleted === "Village / Town") {
                $('#sddistrictnew' + i + '').val('').trigger('change');
            }
            if ((clickpopup == 'Addition') && data.value == '' && seleted === "Village / Town") {
                $('#name2021' + i + '').val('').trigger('change');
            }
            // JC_35 srikanth 
            else if ((clickpopup == 'Create') && data.value != '' && seleted === "Village / Town") {
                $('#sddistrictnew' + i + '').val('').trigger('change');
            ...
            else if ((clickpopup == 'Rename') && data.value != '' && seleted === "Village / Town") {
                $('#sddistrictnew' + i + '').val('').trigger('change');
                $('#named2021' + i + '').val('').trigger('change');
            }
            //
    ...
    //srikanth jc_35
    if ((clickpopup == 'Rename') && data.value != '' && seleted === "Village / Town") {
        $('#sddistrictnew' + i + '').val('').trigger('change');
    }
    //srikanth jc_35 1
    if ((clickpopup == 'Addition') && data.value != '' && seleted === "Village / Town") {
...
function get_sddistrict_popupto(data, clickpopup) {
    ...
    if ((clickpopup == 'Create' || clickpopup == 'Merge' || clickpopup == 'Partiallysm' || clickpopup == 'Addition' || clickpopup == 'Rename' || clickpopup == 'Deletion' || clickpopup == 'Reshuffle') && data.value != '') {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopup_to_sd&comefrom=" + seleted + "&selectdtid=" + data.value + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids
        }).done(function (result) {
            ...
            if (finalresult[1] == 'Village / Town') {
                //   alert('innnnnnnn');
                if (clickpopup == 'Reshuffle') {
                    var fromdata = $('select[name="sddistrictget[]"] option:selected').map(function () {
                        return this.value;
                ...
                //  $('#sddistrictnew'+finalresult[3]+'').val('').trigger('change');
                // JC_11 Modified by Arul for disable add M/PM
                if (namefrom.length > 0 && action.length > 0 && statenew.length > 0 && stateid == '' && (clickpopup == 'Reshuffle' || clickpopup == 'Merge')) {
                    DisableRow(rn);
                }
                // Ends... 
    ...
    else if (data.value == '' && (clickpopup == 'Rename' || clickpopup == 'Merge' || clickpopup == 'Reshuffle') && seleted == 'Sub-District') {
        if (clickpopup == 'Rename' || clickpopup == 'Merge') {
            $('#named2021').val('').trigger('change');
            $('#newnamecheck').val('').trigger('change');
        }
        // JC_007 Modified by Arul for disable add M/PM
        if (namefrom.length > 0 && action.length > 0 && statenew.length > 0 && stateid == '' && (clickpopup == 'Reshuffle' || clickpopup == 'Merge')) {
            EnableRow(rn);
        }
        // Ends... 
    ...
    // Action 2021 trigger Arul  
    else if (data.value == '' && clickpopup == 'Create' && seleted == "Sub-District") {
        $('#name2021').val('').trigger('keyup');
    }
    else if (data.value == '' && clickpopup == 'Create' && seleted == "Village / Town") {
        $('#sddistrictnew1').val('').trigger('change');
    ...
    // JC_11 Modified by Arul for disable add M/PM
    else if (data.value == '' && (clickpopup == 'Reshuffle' || clickpopup == 'Merge') && seleted == "Village / Town") {
        if (namefrom.length > 0 && action.length > 0 && statenew.length > 0 && stateid == '') {
            EnableRow(rn);
        }
    }
    // Ends... 
...
function get_district_popup_distdata(data, clickpopup) {
    ...
    if ((clickpopup == 'Create' || clickpopup == 'Merge' || clickpopup == 'Partiallysm' || clickpopup == 'Reshuffle')) {
        ...
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=getdataofpopup1&comefrom=" + seleted + "&selectstid=" + data.value + "&clickpopup=" + clickpopup + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids
        }).done(function (result) {
            ...
            if (finalresult[1] == 'Sub-District') {
                ...
                $('#comefromdata').html('');
                $("#comefromdata").html('<select class="form-select  mainvaldata"  name = "namefrom[]" id="selected_come" onchange="return get_fromvalue1(this.value,1)" ><option value="">Select Village / Town</option></select>');
                $('select').select2();
                $(".AC").css({ 'margin-left': '' });
        ...
        if (seleted == 'Village / Town') {
            ...
            var req = '';
            if ((clickpopup == 'Rename' || clickpopup == 'Addition' || clickpopup == 'Deletion')) {
                req = '';
            }
            else {
                req = 'required';
...
function createnew(createfrom) {
    ...
    if (seleted == 'District') {
        ...
        if (createfrom == 'Addition' || createfrom == 'Rename' || createfrom == 'Deletion') {
            $(".fromstate").prop('required', false);
            $(".districtstatus").prop('required', false);
            $(".sddistrictstatus").prop('required', false);
            $(".mainvaldata").prop('required', false);
    ...
    $.ajax({
        type: "POST",
        url: "insert_data.php",
        data: "formname=getdataofpopup&comefrom=" + seleted + "&fstids=" + fstids + "&fdtids=" + fdtids + "&fsdids=" + fsdids + "&tstids=" + tstids + "&tdtids=" + tdtids + "&tsdids=" + tsdids + "&createfrom=" + createfrom + "&clickpopup=" + createfrom
    }).done(function (result) {
        ...
        if (createfrom == 'Create' || createfrom == 'Addition') {
            ...
            if (seleted == 'State') {
                $("#maintitle").html("Rename/Status Change " + finalresult[2] + " / UT");
                $("#comespan").html("[Rename/Status Change " + finalresult[2] + " / UT]");
                $("#addlable").html("Select " + finalresult[2] + " / UT");
                $("#addlable1").html("Select " + finalresult[2] + " / UT");
                $("#name2021").attr("placeholder", "" + finalresult[2] + " / UT Name");
            }
        ...
        if (flagof == 'true') {
            ...
            if (createfrom == 'Create' || createfrom == 'Addition' || createfrom == 'Deletion')
            // if(createfrom=='Create' || createfrom=='Addition')
            {
                // if(createfrom=='Addition')
        ...
        if (finalresult[2] == 'District' || finalresult[2] == 'State') {
            ...
            if (finalresult[2] == 'Village / Town') {
                ...
                $("#comefromdata").html('<select class="form-select  mainvaldata " ' + jig + ' name = "namefrom[]" id="selected_come" onchange="return get_fromvalue1(this.value,1)" ></select>');
                $('select').select2();
                $(".AC").css({ 'margin-left': '' });
            }
            else {
            ...
            if (finalresult[8] != '') {
                ...
                else if (stateid != '' && (createfrom == 'Rename' || createfrom == 'Addition' || createfrom == 'Deletion')) {
                    $('#statenew1').val(stateid).trigger('change');
                }
                else {
                    $('#statenew1').val(finalresult[9]).trigger('change');
            ...
            if (createfrom == 'Rename' && (finalresult[2] == 'State' || finalresult[2] == 'Village / Town')) {
...
function psmergenew(createfrom) {
    ...
    $.ajax({
        type: "POST",
        url: "insert_data.php",
        data: "formname=getdataofpopup&comefrom=" + seleted + "&selectstid=" + selectstid + "&dtstname=" + dtstname + "&dtselected=" + dtselected + "&selectstidupdated=" + selectstidupdated + "&createfrom=" + createfrom + "&sdidsselected=" + sdidsselected
    }).done(function (result) {
        ...
        if (finalresult[2] == 'District' || finalresult[2] == 'State') {
            ...
            if (finalresult[2] == 'State') {
                $("#selected_comemrgp").append($('<option>', {
                    value: '',
                    text: 'Select ' + finalresult[2] + ' / UT',
                }));
            ...
            $("#actionmrgp1,#actionamrgp2").append($('<option>', {
                value: '',
                text: 'Select Action',
            }));
            $(JSON.parse(finalresult[1])).each(function () {
            ...
            if (createfrom == 'Merge') {
                ...
                $('#actionmrgp1,#actionamrgp2').val('Partially Split & Merge').trigger('change');
            }
...
function redirectcompleted(dataval, comefromdoc, doctype) {
    if (dataval !== '') {
        $.ajax({
            type: "POST",
            url: "insert_data.php",
            data: "formname=olddocselect&id=" + dataval + "&flag=drop&comefromdoc=" + comefromdoc
        }).done(function (result) {
            ...
            if (res[0] === 'adddata') {
                ...
                if (comefromdoc === 'comefromdocadd') {
                    ...
                    if (doctype === 'Resolution') {
                        $('#adddoctype option[value="Resolution"]').text('Resolution (Link Document)');
                        $('#adddoctype option[value="Resolution"]').val('Resolution (Link Document)');
                    } else if (doctype === 'Clarification') {
                        $('#adddoctype option[value="Clarification"]').text('Clarification (Link Document)');
                        $('#adddoctype option[value="Clarification"]').val('Clarification (Link Document)');
                    } else if (doctype === 'Collector Letter') {
                        $('#adddoctype option[value="Collector Letter"]').text('Collector Letter (Link Document)');
                        $('#adddoctype option[value="Collector Letter"]').val('Collector Letter (Link Document)');
                    } else if (doctype === 'Others') {
                        $('#adddoctype option[value="Others"]').text('Others (Link Document)');
                        $('#adddoctype option[value="Others"]').val('Others (Link Document)');
...
$("#name2021").keyup(function () {
    ...
    if ((come == 'Village / Town')) {
        ...
        if (value.length > 0 && namefrom.length >= 1 && sddistrictnew.length >= 1 && action.length >= 1 && clickpopup == 'Create') {
            ...
            EnableRow(rn);
            DisableAddButton2();
            $('#vStatus2021_1').val('').trigger('change');
        }
        else if (clickpopup == 'Addition' && sddistrictnew.length == 1 && value.length > 0) {
            EnableAddButton2();
        } else if (clickpopup == 'Addition' && sddistrictnew.length == 0 && value.length == 0) {
...
function handleClick(ck, i, popup) {
    ...
    if (popup == 'submerge') {
        var valueof = $('select[name="selected_comesub[]"] option:selected').map(function () {
            return this.value;

        }).get();

        var valueoftext = $('select[name="selected_comesub[]"] option:selected').map(function () {
            return this.text;

        }).get();
    }
    else {
        var valueof = $('select[name="addlinksDTID' + i + '[]"] option:selected').map(function () {
            return this.value;

        }).get();

        var valueoftext = $('select[name="addlinksDTID' + i + '[]"] option:selected').map(function () {
            return this.text;
    ...
    if (ck.checked == true) {
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                ...
                if (popup == 'submerge') {
                    $("#ms-selected_comesub [class*=ms-elem-selectable]").addClass("disabled");
                    $("#ms-selected_comesub [class*=ms-elem-selection]").addClass("disabled");
                }
                else {
                    $("#ms-addlinksDTID_" + res[1] + " [class*=ms-elem-selectable]").addClass("disabled");
                    $("#ms-addlinksDTID_" + res[1] + " [class*=ms-elem-selection]").addClass("disabled");
                }
                ...
                $('#partiallylevel' + res[1] + '').multiSelect({
                    ...
                    afterSelect: function (values) {
                        ...
                        $('#totaldefultselectedpar_' + i + '').html($el.find('[class*="ms-elem-selection ms-selected"]').length);
        ...
        if (popup == 'submerge') {
            ...
            $("#ms-selected_comesub [class*=ms-elem-selectable]").removeClass("disabled");
            $("#ms-selected_comesub [class*=ms-elem-selection]").removeClass("disabled");

        }
        else {
            $('#addlinksDTID_' + i + '').attr("disabled", false);

            $("#ms-addlinksDTID_" + i + " [class*=ms-elem-selectable]").removeClass("disabled");
            $("#ms-addlinksDTID_" + i + " [class*=ms-elem-selection]").removeClass("disabled");
            ...
            for (var m = 0; m < valueofcheck.length; m++) {
                ...
                if (i != m) {
                    ...
                    $('#ms-addlinksDTID_' + m + '').find('[class*="ms-elem-selectable"]').each(function () {
                    ...
                    $('#ms-addlinksDTID_' + i + '').find('[class*="ms-elem-selectable"]').each(function () {
...
function handleClickdataof(ck, i) {
    ...
    var valueof = $('select[name="partiallylevel' + i + '[]"] option:selected').map(function () {
        return this.value;

    }).get();

    var valueoftext = $('select[name="partiallylevel' + i + '[]"] option:selected').map(function () {
        return this.text;
...
function getactiondata(thisdata, i, dist) {
    ...
    if (thisdata != '') {
        if (thisdata == 'Split') {
            $('#actionevent' + dist + '').html('<input type="text" id="newdist' + dist + '" name="newdist[]" class="form-control">');
        }
        else {
...
$(function () {
    ...
    $('#districts-units-datatable tbody').on('click', 'tr td.btnlink', function (event) {
    ...
    $('#subdistricts-units-datatable tbody').on('click', 'tr td.btnlink', function (event) {

        $("a.dropdown-toggle").dropdown();
        return false;
    });

    $('#villages-units-datatable tbody').on('click', 'tr td.btnlink', function (event) {

        $("a.dropdown-toggle").dropdown();
        return false;
    });

    $('#wards-units-datatable tbody').on('click', 'tr td.btnlink', function (event) {
    ...
    $('#submergedocument .close,#submergedocument .closepop1').on('click', function () {
    ...
    $('#assigndatamergep .close,#assigndatamergep .closepop1').on('click', function () {
    ...
    $('#addnewdocument .close,#addnewdocument .closepop1').on('click', function () {

        $('#addnewdocument').find('input, textarea, button, select').removeAttr('disabled', 'disabled');
    ...
    $('#mergenewdocument .close,#mergenewdocument .closepop1').on('click', function () {
    ...
    $('#units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnlink', function (event) {
            ...
            "ajax": {
                ...
                ,
                {
                    ...
                    render: function (dataa, type, row, meta) {
                        ...
                        if (type === 'display') {
                            data = '<a href="Alldocuments/' + row[2] + '/' + encodeURIComponent(dataa) + '" target="_blank"><i class="fas fa-file-alt fa-2x mb-2" data-toggle="tooltip" data-placement="top" title="" data-original-title=""></i></a>';
                        }
                        return data;
                    }
    ...
    $('#districts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnlink', function (event) {
    ...
    $('#subdistricts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnlink', function (event) {
        $('#dataids').val('');
        var data = JSON.parse(event.target.dataset.todo);
    ...
    // JIGAR LINK OPEN POPUP
    $('#villages-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnlink', function (event) {
    ...
    // JIGAR LINK OPEN POPUP
    $('#wards-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnlink', function (event) {
    ...
    // LINKEDJIGAR
    $('#units-datatable tbody').on('click', 'tr td.btnlinked , tr td a.btnlinked', function (event) {
        // $('#con-close-modal-link').modal('show');
            ...
            'columnDefs': [
                {
                    ...
                    render: function (dataa, type, row, meta) {
                        ...
                        if (type === 'display') {
                            data = '<a href="Alldocuments/' + row[1] + '/' + encodeURIComponent(dataa) + '" target="_blank"><i class="fas fa-file-alt fa-2x mb-2" data-toggle="tooltip" data-placement="top" title="" data-original-title=""></i></a>';
                        }
                        return data;
                    }
    ...
    // LINKEDJIGAR
    $('#districts-units-datatable tbody').on('click', 'tr td.btnlinked , tr td a.btnlinked', function (event) {
        // $('#con-close-modal-link').modal('show');
    ...
    // LINKEDJIGAR
    $('#subdistricts-units-datatable tbody').on('click', 'tr td.btnlinked , tr td a.btnlinked', function (event) {
        // $('#con-close-modal-link').modal('show');
    ...
    // LINKEDJIGAR
    $('#villages-units-datatable tbody').on('click', 'tr td.btnlinked , tr td a.btnlinked', function (event) {
        // $('#con-close-modal-link').modal('show');
    ...
    // LINKEDJIGAR
    $('#wards-units-datatable tbody').on('click', 'tr td.btnlinked , tr td a.btnlinked', function (event) {
        // $('#con-close-modal-link').modal('show');
    ...
    $('#con-close-modal-add .close,#con-close-modal-add .closepop').on('click', function () {
    ...
    $('#con-close-modal-linked .close,#con-close-modal-linked .closepop').on('click', function () {
    ...
    $('#con-close-modal-linkdc .close,#con-close-modal-linkdc .closepop').on('click', function () {

        $('#con-close-modal-linkdc').find('form')[0].reset();
        $('#con-close-modal-linkdc').find('form select').select2().trigger('change');
    });
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                if (data == 'login') {
                    ...
                    $("#success_alert").fadeTo(4000, 500).slideUp(500, function () {
                    ...
                    $("#danger_alert_msg").text("Your account not activated. Contact to admin.");
                    $("#danger_alert").fadeTo(4000, 500).slideUp(500, function () {
                    ...
                    $("#danger_alert_msg").text("Your Login Id  or Password is incorrect.");
                    $("#danger_alert").fadeTo(4000, 500).slideUp(500, function () {
                        $("#danger_alert").slideUp(500);
                    });
                    ...
                    // Command: toastr["error"]("Your email address or password is incorrect. Please try again.")
                    $("#danger_alert_msg").text("Your email address or password is incorrect.");
                    $("#danger_alert").fadeTo(4000, 500).slideUp(500, function () {
                        $("#danger_alert").slideUp(500);
                    });
                    ...
                    $("#danger_alert_msg").text("Your Login ID and Password is incorrect.");
                    $("#danger_alert").fadeTo(4000, 500).slideUp(500, function () {
                        $("#danger_alert").slideUp(500);
                    });
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                if (data == 'email_send_of') {
                    ...
                    $("#success_alert_msg_for").text("Your Password has been sent to your email.");
                    $("#success_alert_for").fadeTo(2000, 500).slideUp(500, function () {
                        $("#success_alert_for").slideUp(500);
                    });


                    setTimeout('$("#forgot").hide();$("#login_form").parsley().reset();$("#login_form")[0].reset();$("#login").show();', '3000');
                    ...
                    $("#danger_alert_msg_for").text("Your email does not exist in our records.");
                    $("#danger_alert_for").fadeTo(4000, 500).slideUp(500, function () {
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                if (data == 'changepassword') {
                    ...
                    Command: toastr["warning"]("Password should be at least 8 to 16 characters in length and should include at least one upper case letter, one lower case letter, one number, and one special character.");
                    return false;
                }
                    ...
                    Command: toastr["warning"]("New password is similar to the last 2 passwords.Kindly use unique one");
                    return false;
                }
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                if (data == 'profileupdatedone') {
                    ...
                    Command: toastr["error"]("This Mobile Number already exists. Use another to process further.");
                    return false;
    ...
    $("#linkdata").submit(function (e) {
        ...
        var rows_selected = $('#jigardata-datatable').DataTable().column(0).checkboxes.selected();
    ...
    $("#linkeddataupdate").submit(function (e) {
        ...
        var rows_selected = $('#linked-datatable').DataTable().column(0).checkboxes.selected();
    ...
    $("#assigndata").submit(function (e) {
        ...
        if ($('#comefromcheck').val() == 'Village / Town' || $('#clickpopup').val() == 'Reshuffle') {
            ...
            for (var h = 0; h < resac.length; h++) {
                if (h == 0) {
                    ...
                    $('select[id="id2021' + resac[h] + '"] option:selected').each(function () {
                        if ($(this).val() != '') {
                            namefromtext.push(this.text);
                        }
            ...
            var namefromtext = $('select[name="namefrom[]"] option:selected').map(function () {
                return this.text;
        ...
        if ($('#clickpopup').val() == 'Reshuffle' && $('#comefromcheck').val() == 'Village / Town') {
            var namefrompre = $('select[name="sddistrictnew[]"] option:selected').map(function () {
                return this.text;

            }).get();
        }
        else {
            var namefrompre = $('select[name="districtnew[]"] option:selected').map(function () {
                return this.text;
        ...
        var statenew = $('select[name="statenew[]"] option:selected').map(function () {
            return this.text;

        }).get();

        var districtnew = $('select[name="districtnew[]"] option:selected').map(function () {
            return this.text;

        }).get();


        var sddistrictnew = $('select[name="sddistrictnew[]"] option:selected').map(function () {
            return this.text;

        }).get();


        var statenewfrom = $('select[name="fromstate[]"] option:selected').map(function () {
            return this.text;

        }).get();

        var districtnewfrom = $('select[name="districtget[]"] option:selected').map(function () {
            return this.text;

        }).get();


        var sddistrictnewfrom = $('select[name="sddistrictget[]"] option:selected').map(function () {
            return this.text;
        ...
        //Delete Rithisha
        if ($('#clickpopup').val() == 'Merge' || $('#clickpopup').val() == 'Partiallysm' || $('#clickpopup').val() == 'Rename' || $('#clickpopup').val() == 'Deletion') {


            var nametotext = $('select[name="newnamem[]"] option:selected').map(function () {
                return this.text;
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                ...
                if (res[0] == 'adddata') {
                    ...
                    if ($('#clickpopup').val() == 'Merge' || $('#clickpopup').val() == 'Partiallysm') {
                        ...
                        if (res[4] == 0) {
                            ...
                            if (dataarray['comefromcheck'] == 'State') {
                                ...
                                for (var k = 0; k < dataarray['namefrom'].length; k++) {
                                    ...
                                    if (dataarray['ostate'][k] == 'ST') {
                                        namefromtost = '<span class="trinstrong1">' + namefromtext[k] + '</span>' + finalflag + '';
                                        fromnamedata += '<strong>' + namefromtost + '</strong>, ';
                                    }
                                    else {
                                        namefromtout = ' <span class="trinstrong1">' + namefromtext[k] + '</span>' + finalflag + '';
                                ...
                                if (totalcount != 0 && totalucount == 0) {
                                    fmsg = '(Total - ' + totalcount + ' ) - <strong>' + flagstatusst + '(s) : </strong>' + fromnamedata + '  <br> ' + $('#clickpopup').val() + 'd into <br><span class="trinspam"><strong>' + sssflagto + ' : </strong></span> <strong class="trinstrong">' + dataarray['nametotext'] + '</strong><strong>' + newflagof + '</strong>';
                                }
                                else if (totalcount == 0 && totalucount != 0) {
                                    fmsg = '(Total - ' + totalucount + ' ) - <strong>' + flagstatusut + '(s) : </strong>' + fromnamedataut + '  <br> ' + $('#clickpopup').val() + 'd into <br><span class="trinspam"><strong>' + sssflagto + ' : </strong></span> <strong class="trinstrong">' + dataarray['nametotext'] + '</strong><strong>' + newflagof + '</strong>';
                                }
                                else {
                                    fmsg = '(Total - ' + totalcount + ' ) - <strong>' + flagstatusst + '(s) : </strong>' + fromnamedata + '  <br> AND <br> (Total - ' + totalucount + ' ) - <strong>' + flagstatusut + '(s) : </strong>' + fromnamedataut + '  <br> ' + $('#clickpopup').val() + 'd into <br><strong><span class="trinspam">' + sssflagto + ' : </span></strong> <strong class="trinstrong">' + dataarray['nametotext'] + '</strong><strong>' + newflagof + '</strong>';
                                }
                                ...
                                fmsg = '(Total - ' + totalcount + ' ) - <strong>' + flagstatus + '(s)</strong> : ' + fromnamedata + '  <br> ' + $('#clickpopup').val() + 'd into <br> <strong>' + flagstatus + ' : </strong><strong class="trinstrong">' + dataarray['nametotext'] + '</strong>';
                            ...
                            if (dataarray['oremovenew'] == 1) {
                                ...
                                if (dataarray['comefromcheck'] == 'State') {
                                    //modified by sahana state status in merge flow 1509  
                                    fmsg = fmsg + ' <br>AND<br> <strong>' + $sahana + '</strong><strong class="trinstrong">' + dataarray['nametotext'] + '</strong> name changed to <strong class="trinstrong">' + aa + '</strong><strong>(' + sssflag_n + ')</strong>'; //modiifed by sahana JC_41
                                }
                                else {
                                    //modified by sahana state status in merge flow 1509  
                                    fmsg = fmsg + ' <br>AND<br> <strong>' + dataarray['comefromcheck'] + ': </strong><strong class="trinstrong">' + dataarray['nametotext'] + '</strong> name changed to <strong class="trinstrong">' + aa + ' <span class="trinspam">(' + dataarray['comefromcheck'] + ')</span></strong>'; //modiifed by sahana JC_41
                                }
                            }
                            ...
                            $('#viewerlast').css("display", "block");
                            $('#pdf').css("display", "block")
                            var filename = $('#viewerlast').contents().get(0).location.href.replace(/^.*[\\\/]/, '');

                            $('#docname').text(decodeURIComponent(filename));
                            $('#viewerlaststep').attr('src', $('#viewerlast').contents().get(0).location.href);
                            ...
                            $('' + n + '').multiSelect({
                                ...
                                afterSelect: function (values) {
                                    ...
                                    $('#totaldefultselected_' + checkdata[1] + '').html($el.find('[class*="ms-elem-selection ms-selected"]').length);
                                    $el.find('[class*="ms-elem-selection ms-selected"]').each(function () {
                    ...
                    //code changed by bheema//
                        ...
                        if (dataarray['applyon'] == 'State') {
                            mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead" colspan="2"><th> Existing (State / UT) </th><th>Rename </th><th>Status Change</th></tr></thead><tbody>';
                        }
                        else {
                            ...
                            if (dataarray['applyon'] == 'Village / Town') {
                                ...
                                mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Existing - ' + lableof + '(s)</th><th> Rename </th><th>Status Change</th></tr></thead><tbody>';
                            }
                            else {
                                lableof = dataarray['applyon'];

                                mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Existing - ' + lableof + '(s)</th><th> Rename </th></tr></thead><tbody>';
                            }
                        ...
                        for (var k = 0; k < dataarray['newnamecheck'].length; k++) {
                            ...
                            if (dataarray['applyon'] == 'State') {
                                if (dataarray['toStatus'][k] == 'ST') {
                                    ...
                                    fromnamenew =
                                        '<strong class="trinstrong">' + dataarray["newnamecheck"][k] + "</strong>";


                                    if (dataarray['newnamecheck'][k] != '') {
                                        var hhh = '<strong class="trinstrong">' + dataarray['newnamecheck'][k] + '</strong>';
                                    }
                                    else {
                                    ...
                                    // status change to changed
                                    // fmsg += " <ol>(Total - " + totalvcount +  " ) <strong> " + vflag +' - <span class="trinstrong1">' +toname[k]+"</span></strong>  Renamed as " +hh+  "<br> And status changed to  <b>"  +nst+" </b> </ol> "; 
                                    mainaction += '<tr><td><strong class="trinstrong1">' + toname[k] + ' <span class="trinspam">(State)</span></strong></td><td>' + hhh + '</td><td><strong class="trinstrong"><span class="trinspam">' + nst + '</span></strong></td></tr>';
                                }
                                else {
                                ...
                                if (dataarray['applyon'] == 'Village / Town') {
                                    ...
                                    if (dataarray['vlevel'][k] == dataarray['vStateStatus'][k]) {
                                        if (dataarray['ovstatus'][k] == dataarray['vstatus'][k]) {
                                            ...
                                            fl = '<strong class="trinstrong">' + dataarray['vStateStatus'][k] + ' - ' + dataarray['vstatus'][k] + '</strong>';
                                        }
                                    ...
                                    if (dataarray['newnamecheck'][k] != '') {
                                        ...
                                        var hh = '<strong class="trinstrong">' + dataarray['newnamecheck'][k] + ' </strong>';
                                    }
                                    else {
                                    ...
                                    vflag = ('' + capitalizeFirstLetter(dataarray['vlevel'][k]) + ' - ' + dataarray['ovstatus'][k] + '');


                                    fromnamenew = '<strong class="trinstrong">' + dataarray["newnamecheck"][k] + "</strong>";
                                    // status change to changed one to one village
                                    if (dataarray["newnamecheck"][k] != '' && dataarray['vstatus'][k] == dataarray['ovstatus'][k]) {//rename
                                        //    fmsg +=" <ol>(Total - " + totalvcount + " ) <strong> " +  vflag + ' - <span class="trinstrong1">'   +toname[k]+ "</span></strong>  Renamed as "  +hh+ "</span></strong>";
                                    } else if (dataarray['vstatus'][k] != dataarray['ovstatus'][k] && dataarray["newnamecheck"][k] == '') {//status change
                                        //    fmsg +=" <ol>(Total - " + totalvcount + " ) <strong> " +  vflag + ' - <span class="trinstrong1">'   +toname[k]+ "</span></strong> Status Changed To  " +fl+ " </span></strong>";
                                    }
                                    else { //rename n status change
                                    ...
                                    mainaction += '<tr><td><strong class="trinstrong1">' + toname[k] + ' <span class="trinspam">(' + capitalizeFirstLetter(dataarray['vlevel'][k]) + ' - ' + dataarray['ovstatus'][k] + ')</span></strong></td><td>' + hh + '</td><td><strong class="trinstrong1">' + fl + '</strong></td></tr>';
                                }
                                    ...
                                    mainaction += '<tr><td><strong class="trinstrong1">' + toname[k] + ' <span class="trinspam">(' + dataarray['applyon'] + ')</span></strong></td><td><strong class="trinstrong">' + dataarray['newnamecheck'][k] + ' <span class="trinspam">(' + dataarray['applyon'] + ')</span></strong></td></tr>';
                                }
                            }
                        ...
                        if (dataarray['applyon'] == 'Village / Town') {
                            lableof = 'Village(s) / Town';
                            mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Existing - ' + lableof + '(s)</th><th> Delete </th><th>Status Change</th></tr></thead><tbody>';
                        }
                        else {
                            lableof = dataarray['applyon'];

                            mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Existing - ' + lableof + '(s)</th><th> Delete </th></tr></thead><tbody>';
                        }
                    ...
                    //Reshuffle code for Sub District modified by Rithisha
                        ...
                        {
                            mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Existing -Sub District(s)</th><th> Moved / Reshuffled </th></tr></thead><tbody>';
                        }
                        ...
                        fromnamenew = '<strong class="trinstrong"><strong><span class="trinspam">District: </span></strong>' + dataarray['namefrompre'] + '</strong>';
                        vnames = vnames.slice(0, -2);

                        fmsg = '(Total - ' + totalvcount + ' ) <strong> ' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong> <br> Moved / Reshuffled to <br>' + fromnamenew + '';
                        $('#mainaction').html('');

                        $('#maintitledata').html(fmsg);

                        mainaction += '<tr><td><strong><span class="trinspam">' + dataarray['applyon'] + ':</span></strong> <strong class="trinstrong1">' + vnames + ' </strong></td><td><strong class="trinstrong">' + fromnamenew + ' </strong></td></tr>';
                        ...
                        if (res[6] !== 'true') {
                            ...
                            if (returndata['flag'] == 'newname') {
                                ...
                                fromnamenew = '<strong class="trinstrong">' + capitalizeFirstLetter(returndata['namefromtext']) + '</strong>';
                                //modified by sahana  Full Merge summary message.
                                fmsg = '(Total - 1 ) <strong>New  ' + returndata['comefromcheck'] + '(s) - <span class="trinstrong1">' + returndata['newname'][0] + '</span></strong>  <br> Created from <br><strong><span class="trinspam">' + returndata['comefromcheck'] + '(s) :</span></strong> ' + fromnamenew + '&nbsp;Using Full Merge Action';
                                ...
                                for (var k = 0; k < returndata['namefrom'].length; k++) {
                                    fromnamenew += '<strong class="trinstrong">' + namefromtext[k] + ' </strong> - <strong><span class="trinspam">' + returndata['action'][k] + ', </span></strong> '; //modified by sahana JC_41
                                }


                                fromnamenew = fromnamenew.slice(0, -2);
                                fmsg = '(Total - 1 ) <strong>New  ' + returndata['comefromcheck'] + '(s) - <span class="trinstrong1">' + returndata['newname'][0] + '</span></strong>  <br> Created from <br><strong><span class="trinspam">' + returndata['comefromcheck'] + '(s) :</span></strong> ' + fromnamenew + '';
                    ...
                    if ($('#clickpopup').val() == 'Merge' || $('#clickpopup').val() == 'Partiallysm') {
                        ...
                        if (dataarray['flag'] == 'newname') {
                            for (k = 0; k < dataarray['namefrom'].length; k++) {
                                ...
                                if (dataarray['action'] == 'Merge' || dataarray['action'] == 'Partially Merge') {
                                    if (resArray[0][k]['Level'] == 'VILLAGE') {//modified by sahana 0509
                                        totalvcount = totalvcount + 1;
                                        vnames += dataarray['namefrom'][k] + '&nbsp<span class="trinspam">- ' + dataarray['action'] + 'd&nbsp</span>';
                                        vflag = 'Village(s)';
                                    } else {
                                        totaltcount = totaltcount + 1;
                                        name = dataarray['nametotext'];
                                        tnames += dataarray['namefrom'][k] + '&nbsp<span class="trinspam">- ' + dataarray['action'] + 'd&nbsp</span>';
                                        tflag = 'Town(s)';
                                    }
                                }
                                ...
                                if (dataarray['action'] == 'Merge' || dataarray['action'] == 'Partially Merge') {
                                    ...
                                    if ((dataarray['vStateStatus'][0] == dataarray['vlevel'][0]) && (dataarray['ovstatus'][0] == dataarray['vstatus'][0])) { //modified by sahana 0509
                                    ...
                                    if (dataarray['vStateStatus'][0] == dataarray['vlevel'][0]) { //modified by sahana 0509
                                        vs = dataarray['vStateStatus'][0];
                                    } else {
                                        vs = "Now " + dataarray['vStateStatus'][0];
                                    }
                            ...
                            for (var setIndex = 0; setIndex < namefromSets.length; setIndex++) {

                                var namefromSet = namefromSets[setIndex];
                                var namefromaction = dataarray['action'][setIndex];

                                for (var elementIndex = 0; elementIndex < namefromSet.length; elementIndex++) {

                                    if (namefromaction == 'Merge' || namefromaction == 'Partially Merge') {
                                        if (resArray[setIndex][elementIndex]['Level'] == 'VILLAGE') {
                                            totalvcount = totalvcount + 1;
                                            vnames += (vnames ? ', ' : '') + resArray[setIndex][elementIndex]['VTName'] + '&nbsp<span class="trinspam">-' + namefromaction + 'd&nbsp</span>';
                                            vflag = 'Village(s)';
                                        } else {
                                            totaltcount = totaltcount + 1;
                                            name = dataarray['nametotext'];
                                            tnames += (tnames ? ', ' : '') + resArray[setIndex][elementIndex]['VTName'] + '&nbsp<span class="trinspam">-' + namefromaction + 'd&nbsp</span>';
                                            tflag = 'Town(s)';
                                        }
                                    } else {
                                        if (resArray[setIndex][elementIndex]['Level'] == 'VILLAGE') {
                                    ...
                                    for (var i = 0; i < dataarray['vStateStatus'].length; i++) {
                                        if (dataarray['vStateStatus'][i].trim() === dataarray['vlevel'][i].trim()) {
                        ...
                        fromnamenew = '<strong><span class="trinspam">' + $sahana + ': </strong></span><strong class="trinstrong">' + createddata + '<span class="trinspam"> (' + vs + ' - ' + dataarray['vstatus'] + ')</span></strong>';
                        ...
                        if (totalvcount != 0 && totaltcount == 0) {
                            fmsg = '(Total - ' + totalvcount + ' ) <strong>' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong> <br> into <br> ' + fromnamenew + '';
                        }
                        else if (totalvcount == 0 && totaltcount != 0) {

                            fmsg = '(Total - ' + totaltcount + ' ) <strong>' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong>  <br> into <br> ' + fromnamenew + '';

                        }
                        else {
                            fmsg = '(Total - ' + totalvcount + ' ) <strong>' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong><br> AND <br> (Total - ' + totaltcount + ' ) <strong>' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong>  <br> into <br> ' + fromnamenew + '';
                        ...
                        if (dataarray['newnamecheck'][0] != '') {
                            //modified by sahana status merge village
                            fmsg = fmsg + ' <br> AND <br> <strong><span class="trinspam"></strong></span>' + fromnamenew + ' name changed to <strong class="trinstrong">' + dataarray['newnamecheck'][0] + '<span class="trinspam"> (' + vs + ' - ' + dataarray['vstatus'] + ')</span></strong>'; //modified by sahana JC_41                 
                        }
                        ...
                        for (var k = 0; k < dataarray['newnamecheck'].length; k++) {
                            ...
                            main += ' <strong>' + toname[k] + '</strong> ' + dataarray['comefromcheck'] + ' Rename as <strong>' + dataarray['newnamecheck'][k] + '</strong> and';
                        }
                        ...
                        if (res[5] == 'Addition') {
                            ...
                            //  createddata = createddata.slice(0, -2);
                            fromnamenew = '<strong class="trinstrong">' + createddata + '</strong>';
                            vnames = vnames.slice(0, -2);
                            tnames = tnames.slice(0, -2);


                            if (totalvcount != 0 && totaltcount == 0) {
                                fmsg = '(Total - ' + totalvcount + ' ) <strong>New  ' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong> <br> Added.';
                            }
                            else if (totalvcount == 0 && totaltcount != 0) {

                                fmsg = '(Total - ' + totaltcount + ' ) <strong>New  ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong>  <br> Added.';

                            }
                            else {
                                fmsg = '(Total - ' + totalvcount + ' ) <strong>New  ' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong><br> AND <br> (Total - ' + totaltcount + ' ) <strong>New  ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong>  <br> Added.';
                        ...
                        //Reshuffle code for village/town modified by Rithisha
                            ...
                            mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Existing - Village(s) / Town(s)</th><th> Moved / Reshuffled </th></tr></thead><tbody>';
                            ...
                            createddata = createddata.slice(0, -2);
                            fromnamenew = '<strong class="trinstrong">' + dataarray['namefrompre'] + '</strong>';
                            vnames = vnames.slice(0, -2);
                            tnames = tnames.slice(0, -2);


                            if (totalvcount != 0 && totaltcount == 0) {
                                fmsg = '(Total - ' + totalvcount + ' ) <strong> ' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong> <br> Move / Reshuffled to <br><strong>Sub-District: </strong>' + fromnamenew + '';
                            }
                            else if (totalvcount == 0 && totaltcount != 0) {

                                fmsg = '(Total - ' + totaltcount + ' ) <strong> ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong>  <br> Move / Reshuffled to <br><strong>Sub-District: </strong>' + fromnamenew + '';

                            }
                            else {
                                fmsg = '(Total - ' + totalvcount + ' ) <strong> ' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong><br> AND <br> (Total - ' + totaltcount + ' ) <strong> ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong>  <br> Move / Reshuffled to <br><strong>Sub-District: </strong>' + fromnamenew + '';
                            }

                            if (vnames && !tnames) {
                                mainaction += '<tr><td><strong><span class="trinspam">Village(s): </span></strong><strong class="trinstrong1">' + vnames + ' </strong></td><td><strong>Sub-District: </strong>' + fromnamenew + '</td></tr>';
                            }
                            else if (tnames && !vnames) {
                                mainaction += '<tr><td><strong><span class="trinspam">Town(s): </span></strong><strong class="trinstrong1">' + tnames + ' </strong></td><td><strong>Sub-District: </strong>' + fromnamenew + '</td></tr>';
                            }
                            else {
                                mainaction += '<tr><td><strong><span class="trinspam">Village(s): </span></strong><strong class="trinstrong1">' + vnames + '</strong><br><strong><span class="trinspam">Town(s): </span></strong><strong class="trinstrong1">' + tnames + '</strong> </td><td><strong>Sub-District: </strong>' + fromnamenew + '</td></tr>';
                            }
                        ...
                        //Deletion code to display summary modified by Rithisha
                            ...
                            if (dataarray['applyon'] == 'Village / Town') {
                                ...
                                name = '<strong class="trinstrong">' + dataarray['nametotext'] + '</strong>';

                                fmsg = ' <strong>' + lableof + ' - &nbsp<span class="trinstrong1">' + name + '</span></strong>  <br> Deleted  <br>';
                            ...
                            if (dataarray['flag'] === 'newname' && dataarray['newname'] !== null && Array.isArray(dataarray['newname']) && dataarray['newname'].length > 1 && 'namefrom' in dataarray && dataarray['namefrom'].length === 1) {
                                var fromnamenew = '';
                                var vflag = '';
                                var createddata = '';
                                ...
                                for (var elementIndex = 0; elementIndex < namefromSets[0].length; elementIndex++) {
                                    createddata = namefromSets[0][elementIndex]['VTName'] + '-(' + namefromSets[0][elementIndex]['Status'] + ')' + '<span class="trinspam"> - ' + namefromaction + '</span>';
                                    fromnamenew += '<strong><span class="trinspam">' + capitalizeFirstLetter(namefromSets[0][elementIndex]['Level']) + ': </span></strong><strong class="trinstrong">' + createddata + '</strong>';
                            ...
                            //modified by sahana in village level for differentiating split and full merge 1-1 and m-1 
                                ...
                                for (var k = 0; k < namefromSets.length; k++) {
                                    ...
                                    for (var elementIndex = 0; elementIndex < namefromSets[k].length; elementIndex++) {
                                        createddata = namefromSets[k][elementIndex]['VTName'] + '<span class="trinspam"> - ' + namefromaction + '</span>';
                                        fromnamenew += '<strong class="trinstrong">' + createddata + '</strong>';
                            ...
                            //modified by sahana JC_09 village level split and full merge summary.
                                ...
                                for (var setIndex = 0; setIndex < namefromSets.length; setIndex++) {
                                    ...
                                    for (var elementIndex = 0; elementIndex < namefromSet.length; elementIndex++) {
                                        ...
                                        totalvcount = 1;
                                        vnames = name + ' (' + dataarray['vstatus'][0] + ')';
                                        vflag1 = capitalizeFirstLetter(dataarray['vStateStatus'][0]);
                                        vflag = vflag1 + '(s)';

                                        fromnamenew += '<strong class="trinstrong">' + namefromSet[elementIndex]['VTName'] + '<span class="trinspam"> - ' + namefromaction + '</span></strong>';
                                ...
                                if (fromnamenew.endsWith(' , ')) {
                                    fromnamenew = fromnamenew.substring(0, fromnamenew.length - 3);
                                }
                            }

                            //modified by sahana Split, Full Merge summary message in village level.
                            if (totalvcount != 0 && totaltcount == 0) {
                                if (dataarray['action'].includes('Split') && dataarray['action'].includes('Full Merge')) {
                                    fmsg = '(Total - ' + totalvcount + ') <strong>New ' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong> <br> Created from <br> ' + fromnamenew;
                            ...
                            //modified by sahana Split, Full Merge summary message in town level.
                                ...
                                if (dataarray['action'].includes('Split') && dataarray['action'].includes('Full Merge')) {
                                    fmsg = '(Total - ' + totaltcount + ') <strong>New ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong> <br> Created from <br> ' + fromnamenew;
                                } else if (dataarray['action'].includes('Split')) {
                                    fmsg = '(Total - ' + totaltcount + ') <strong>New ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong> <br> Created from <br> ' + fromnamenew;
                                } else if (dataarray['action'].includes('Full Merge')) {
                                    fmsg = '(Total - ' + totaltcount + ') <strong>New ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong> <br> Created from <br> ' + fromnamenew;
                            ...
                            //modified by sahana creation of village one to many 
                                ...
                                fmsg = '(Total - ' + totalvcount + ') <strong>New ' + vflag + ' - <span class="trinstrong1">' + vnames + '</span></strong> <br> AND <br> (Total - ' + totaltcount + ') <strong>New ' + tflag + ' - <span class="trinstrong1">' + tnames + '</span></strong> <br> Created from <br> ' + fromnamenew;
                            }
    ...
    $("#submergedata").submit(function (e) {
        ...
        var namefromtext = $('select[name="selected_comesub[]"] option:selected').map(function () {
            return this.text;

        }).get();

        namefromtextlevel = $('select[name="partiallylevel0[]"] option:selected').map(function () {
            return this.text;
        ...
        var statenew = $('select[name="stategetsub[]"] option:selected').map(function () {
            return this.text;

        }).get();

        var districtnew = $('select[name="districtgetsub[]"] option:selected').map(function () {
            return this.text;

        }).get();


        var sddistrictnew = $('select[name="subdistrictgetsub[]"] option:selected').map(function () {
            return this.text;
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                ...
                if (res[0] == 'addsddata') {
                    ...
                    if (res[4] != 0 && dataarray['comefromchecksub'] != 'Village / Town') {
                        ...
                        if (dataarray['comefromchecksub'] == 'State') {
                            ...
                            for (var i = 0; i < dataarray['selected_comesub'].length; i++) {
                                ...
                                if (dataarray['stStatus'][i] == 'ST') {
                                    ...
                                    stmsgname += '<strong class="trinstrong1">' + namefromtext[i] + '</strong>, ';
                                }
                                else {
                                    totalutcount = totalutcount + 1;
                                    utflag = 'Union Territory(s)';
                                    //  utmsgname +=namefromtext[i]+', ';
                                    utmsgname += '<strong class="trinstrong1">' + namefromtext[i] + '</strong>, ';
                                }
                            ...
                            if (dataarray['namefromtextlevel'] == '') {
                                ...
                                for (var i = 0; i < dataarray['selected_comesub'].length; i++) {
                                    ...
                                    if (dataarray['vtLevel'][i] == 'VILLAGE') {
                                        ...
                                        if (dataarray['partiallylevel0'].includes(dataarray['selected_comesub'][i])) {
                                            stmsgnameplsm += '<strong class="trinstrong1">' + namefromtextall[i] + '</strong>, ';
                                            pSubmergedflag = '<span class="trinstrong">Partially Submerged :</span>';
                                        }

                                        else {
                                            stmsgname += '<strong class="trinstrong1">' + namefromtextall[i] + '</strong>, ';
                                            Submergedflag = '<span class="trinstrong">Submerged :</span>';
                                        }
                                        ...
                                        if (dataarray['partiallylevel0'].includes(dataarray['selected_comesub'][i])) {
                                            utmsgnameplsm += '<strong class="trinstrong1">' + namefromtextall[i] + '</strong>, ';
                                            tpSubmergedflag = '<span class="trinstrong">Partially Submerged :</span>';
                                        }
                                        else {
                                            utmsgname += '<strong class="trinstrong1">' + namefromtextall[i] + '</strong>, ';
                                            tSubmergedflag = '<span class="trinstrong">Submerged :</span>';
                                        }
                        ...
                        if (dataarray['comefromchecksub'] == 'State' || dataarray['comefromchecksub'] == 'Village / Town') {

                            if (noflag) {

                                if (totalutcount == 0 && totalstcount != 0) {
                                    fmsg = '(Total - ' + totalstcount + ' ) <strong>' + stflag + ' - <br>' + Submergedflag + ' <span class="trinstrong1">' + stmsgname + '</span><br>' + pSubmergedflag + ' <span class="trinstrong1">' + stmsgnameplsm + '</span></strong> <br> into <br> Sea / River / Any Other';
                                }
                                else if (totalstcount == 0 && totalutcount != 0) {

                                    fmsg = '(Total - ' + totalutcount + ' ) <strong>' + utflag + ' - <br>' + tSubmergedflag + ' <span class="trinstrong1">' + utmsgname + '</span><br>' + tpSubmergedflag + ' <span class="trinstrong1">' + utmsgnameplsm + '</span></strong> <br> into <br> Sea / River / Any Other';

                                }
                                else {
                                    fmsg = '(Total - ' + totalstcount + ' ) <strong>' + stflag + ' - <br>' + Submergedflag + ' <span class="trinstrong1">' + stmsgname + '</span><br>' + pSubmergedflag + ' <span class="trinstrong1">' + stmsgnameplsm + '</span></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong>' + utflag + ' - <br>' + tSubmergedflag + ' <span class="trinstrong1">' + utmsgname + '</span><br>' + tpSubmergedflag + ' <span class="trinstrong1">' + utmsgnameplsm + '</span></strong> <br> into <br> Sea / River / Any Other';
                                ...
                                if (totalutcount == 0 && totalstcount != 0) {
                                    fmsg = '(Total - ' + totalstcount + ' ) <strong>' + stflag + ' - <span class="trinstrong1">' + stmsgname + '</span></strong> <br> Submerged into <br> Sea / River / Any Other';
                                }
                                else if (totalstcount == 0 && totalutcount != 0) {

                                    fmsg = '(Total - ' + totalutcount + ' ) <strong>' + utflag + ' - <span class="trinstrong1">' + utmsgname + '</span></strong>  <br> Submerged into <br> Sea / River / Any Other';

                                }
                                else {
                                    fmsg = '(Total - ' + totalstcount + ' ) <strong>' + stflag + ' - <span class="trinstrong1">' + stmsgname + '</span></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong>' + utflag + ' - <span class="trinstrong1">' + utmsgname + '</span></strong> <br> Submerged into <br> Sea / River / Any Other';
                            ...
                            fmsg = '(Total - ' + dataarray['selected_comesub'].length + ' ) <strong>' + dataarray['comefromchecksub'] + '(s) - <span class="trinstrong1">' + dataarray['namefromtext'] + '</span></strong> <br> Submerged into <br> Sea / River / Any Other';
    ...
    $("#assigndatamerge").submit(function (e) {
        ...
        var namefromtext = $('select[name="namefrommrg[]"] option:selected').map(function () {
            return this.text;

        }).get();

        var nametotext = $('select[name="newnamemrg[]"] option:selected').map(function () {
            return this.text;
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                ...
                if (res[0] == 'addstate') {
                    ...
                    if (dataarray['oremovemrg'] == 1) {
                        var maintitaldata = '<strong>' + name + '</strong> ' + dataarray['comefromcheckmrg'] + ' ' + dataarray['actionmrg'][0] + ' into <strong>' + dataarray['namefromtextmrg'] + '</strong> ' + dataarray['comefromcheckmrg'] + ' And ' + dataarray['comefromcheckmrg'] + ' <strong>' + dataarray['namefromtextmrg'] + '</strong> name changed to <strong>' + dataarray['newnamecheck'][0] + '</strong>';
                    }
                    else {
                        var maintitaldata = '<strong>' + name + '</strong> ' + dataarray['comefromcheckmrg'] + ' ' + dataarray['actionmrg'][0] + ' into <strong>' + dataarray['namefromtextmrg'] + '</strong> ' + dataarray['comefromcheckmrg'];
                    }
                    ...
                    $('#clickbutton').val('Merge');
                    if (dataarray['oremovemrg'] == 1) {
                        var maintitaldata = '<strong> ' + nameto + ' </strong> ' + dataarray['comefromcheckmrg'] + ' ' + dataarray['actionmrg'][0] + ' into <strong>' + dataarray['namefromtextmrg'] + '</strong> ' + dataarray['comefromcheckmrg'] + ' And ' + dataarray['comefromcheckmrg'] + ' <strong>' + dataarray['namefromtextmrg'] + '</strong> name changed to <strong>' + dataarray['newnamecheck'][0] + '</strong>';
                    }
                    else {
                        var maintitaldata = '<strong> ' + nameto + ' </strong> ' + dataarray['comefromcheckmrg'] + ' ' + dataarray['actionmrg'][0] + ' into <strong>' + dataarray['namefromtextmrg'] + '</strong> ' + dataarray['comefromcheckmrg'];
                    }
    ...
    $("#assigndatamergep").submit(function (e) {
        ...
        var namefromtext = $('select[name="namefrommrgp[]"] option:selected').map(function () {
            return this.text;

        }).get();

        var nametotext = $('select[name="newnamemrgp[]"] option:selected').map(function () {
            return this.text;
    ...
    $("#adddocumentnext").submit(function (e) {
        ...
        if (dataarray.hasOwnProperty('flag')) {
            ...
            for (var j = 0; j < dataarray[dataarray['flag']].length; j++) {
                ...
                namefromtext = $('select[name="addlinksDTID' + j + '[]"] option:selected').map(function () {
                    return this.text;
                ...
                if ($('input[name="haveapartially' + j + '[]"]:checked')) {
                    namefromtextp = $('select[name="partiallylevel' + j + '[]"] option:selected').map(function () {
                        return this.text;
            ...
            for (var j = 0; j < dataarray['partiallylevel0'].length; j++) {
                var namefromtext = $('select[name="addlinksDTID' + j + '[]"] option:selected').map(function () {
                    return this.text;
        ...
        if (tempflag != true) {
            ...
            if(namefromtext.length != 0){
                ...
                if (confirm("You are processing this Action without choosing the records from all the blocks. Would you like to continue?") == true) {
                    adddocumentnextsubmit();
                } else {
                    e.preventDefault();
                }
            }
            function adddocumentnextsubmit(){
            // Ends...
                    toastr.options = {
                        "closeButton": false,
                        "debug": false,
                    ...
                    $.ajax({
                        ...
                        success: function (data)   // A function to be called if request succeeds
                        {
                            ...
                            if (returndata.hasOwnProperty('flag')) {
                                ...
                                if (dataarray['applyon'] == 'State') {
                                    mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> New Created State / UT </th><th>Assigned District(s)</th><th>Assigned Partially District(s)</th></tr></thead><tbody>';
                                }
                                else {
    
                                    mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> New Created ' + dataarray['applyon'] + '</th><th>Assigned ' + come + '(s)</th><th>Assigned Partially ' + come + '(s)</th></tr></thead><tbody>';
                                }
                                ...
                                if (returndata['flag'] == 'namefrom') {
                                    ...
                                    for (var k = 0; k < returndata['namefrom'].length; k++) {
                                        ...
                                        if (dataarray['applyon'] == 'State') {
                                            ...
                                            if (returndata['ostate'][k] == 'ST') {
                                                ...
                                                flagstatusut = '<span class="trinspam">Union Territory(s)</span>: ';
                                            ...
                                            if (returndata['ostate'][k] == 'ST') {
                                                ...
                                                fromnamenewst += '<strong class="trinstrong">' + namefromtext[k] + ' ' + finalstatus + ' </strong>, ';
                                            }
                                            else {
    
                                                fromnamenewut += '<strong class="trinstrong">' + namefromtext[k] + ' ' + finalstatus + '</strong>, ';
                                            }
                                            ...
                                            if (dataarray['addlinksDTIDtext' + k + ''] != '') {
                                                //15-09
                                                fromdatacoccatename += '<strong><span class="trinspam"> ' + flagstatus + ': </span></strong><strong class="trinstrong">' + namefromtext[k] + ' - </strong>' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '<br>';
                                            ...
                                            if (dataarray['addpartiallyleveltext' + k + ''] != '') {
                                                fromdatacoccatenamep += '<strong><span class="trinspam">' + flagstatus + ': </span></strong><strong class="trinstrong">' + namefromtext[k] + ' - </strong>' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '<br>';
                                            ...
                                            if (k === returndata['namefrom'].length - 1) {
                                                ...
                                                fromnamenew += '<strong class="trinstrong">' + namefromtext[k] + '</strong> <span class="trinspam"><strong>- ' + returndata['action'][k] + ', &nbsp</strong></span>';
                                            }
    
    
                                            if (dataarray['addlinksDTIDtext' + k + ''] != '') {
                                                //modified by sahana status split district 1609
                                                fromdatacoccatename += '<strong class="trinstrong"><span class="trinspam">' + dataarray['applyon'] + ': </span>' + namefromtext[k] + ' - </strong>' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '<br>';
                                            ...
                                            if (dataarray['addpartiallyleveltext' + k + ''] != '') {
                                                fromdatacoccatenamep += '<strong class="trinstrong"><span class="trinspam">' + dataarray['applyon'] + ': </span>' + namefromtext[k] + ' - </strong>' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '<br>';
                                    ...
                                    if (dataarray['applyon'] == 'State') {
                                        ...
                                        mainaction += '<tr class="trleftalign"><td><strong><span class="trinspam">' + sssflag + '</span><span class="trinstrong1">' + returndata['newname'][0] + '</span></strong></td><td>' + fromdatacoccatename + '</td><td>' + fromdatacoccatenamep + '</td></tr>';
                                    }
                                    else {
                                        //modified by sahana status split district 1609
                                        mainaction += '<tr class="trleftalign"><td><strong><span class="trinspam">' + dataarray['applyon'] + ': </span><span class="trinstrong1">' + returndata['newname'][0] + '</span></strong></td><td>' + fromdatacoccatename + '</td><td>' + fromdatacoccatenamep + '</td></tr>';
                                    }
                                    ...
                                    for (var k = 0; k < returndata['newname'].length; k++) {
                                        if (dataarray['applyon'] == 'State') {
                                            ...
                                            fromnamenewst = '<strong class="trinstrong"><span class="trinspam">' + flagstatus + ' : </span>' + namefromtext[0] + '<span class="trinspam">' + finalstatus + '</span></strong>, ';
    
                                            var flagof = '';
                                            if (dataarray['applyon'] == 'State') {
                                                if (dataarray.hasOwnProperty('partiallylevel' + k + '')) {  //modified by sahana status split district 1609
                                                    flagof += '<strong class="trinstrong"><span class="trinspam">' + flagstatus + ': </span>' + namefromtext[0] + '</strong> - ' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '';
    
                                                }
                                            } else {
                                                if (dataarray.hasOwnProperty('partiallylevel' + k + '')) {  //modified by sahana status split district 1609
                                                    flagof += '<strong class="trinstrong"><span class="trinspam">' + dataarray['applyon'] + ': </span>' + namefromtext[0] + '</strong> - ' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '';
    
                                                }
                                            }
    
                                            mainaction += '<tr class="trleftalign"><td><strong><span class="trinspam">' + sssflag + '</span><span class="trinstrong1">' + returndata['newname'][k] + '</span></strong></td><td><strong class="trinstrong">'; //modified by sahana status of state summary part
                                            if (dataarray['addlinksDTIDtext' + k + ''].length > 0) {
                                                mainaction += '<span class="trinspam">' + flagstatus + ' : </span>' + namefromtext[0] + " - "; //modified by sahana status of state summary part
                                            }
    
                                            mainaction += '</strong> ' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '</td><td>' + flagof + '</td></tr>';
                                            ...
                                            fromnamenew = '<strong class="trinstrong">' + namefromtext[0] + '</strong>, ';
    
                                            //modified by sahana status split district 1509
                                            mainaction += '<tr class="trleftalign"><td><strong><span class="trinspam">' + dataarray['applyon'] + ': </span><span class="trinstrong1">' + returndata['newname'][k] + '</span></strong></td><td><strong class="trinstrong">';
                                            if (dataarray['addlinksDTIDtext' + k + ''].length > 0) {
                                                mainaction += '<strong><span class="trinspam">' + dataarray['applyon'] + ':</span></strong> ' + namefromtext[0] + " - ";
                                            }
                                            mainaction += '</strong>' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '</td><td>' + flagof + '</td></tr>';
                                ...
                                if (dataarray['applyon'] == 'State') {
                                    ...
                                    if (totalutcount == 0 && totalstcount != 0) {
                                        ...
                                        fmsg = '(Total - ' + totalstcount + ' ) <strong>New  ' + stflag + ' - <span class="trinstrong1">' + stmsgname + '</span></strong> <br> Created from <br><strong>' + flagstatusst + '</strong>' + fromnamenewst + '<br><strong>' + flagstatusut + '</strong>' + fromnamenewut + '<br>&nbsp;Using Split Action';
    
                                    }
                                    else if (totalstcount == 0 && totalutcount != 0) {
    
                                        fmsg = '(Total - ' + totalutcount + ' ) <strong>New  ' + utflag + ' - <span class="trinstrong1">' + utmsgname + '</span></strong>  <br> Created from <br><strong>' + flagstatusst + '</strong>' + fromnamenewst + '<br><strong>' + flagstatusut + '</strong>' + fromnamenewut + '<br>&nbsp;Using Split Action';
    
                                    }
                                    else {
                                        fmsg = '(Total - ' + totalstcount + ' ) <strong>New  ' + stflag + ' - <span class="trinstrong1">' + stmsgname + '</span></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong>New  ' + utflag + ' - <span class="trinstrong1">' + utmsgname + '</span></strong>  <br> Created from <br> ' + fromnamenewst + '<br>&nbsp;Using Split Action';
    
                                    }
    
                                }
                                else {
                                    if ((dataarray['applyon'] == 'District') || (dataarray['applyon'] == 'Sub-District')) {
                                        if (returndata['namefrom'].length == 1) {
                                            fmsg = '(Total - ' + totalstcount + ' ) <strong>New  ' + dataarray['applyon'] + '(s) - <span class="trinstrong1">' + stmsgname + '</span></strong>  <br> Created from <br><strong><span class="trinspam">' + dataarray['applyon'] + '(s) :</span></strong> ' + fromnamenew + '<br>&nbsp;Using Split Action';
                                        }
                                        else {
                                            fmsg = '(Total - ' + totalstcount + ' ) <strong>New  ' + dataarray['applyon'] + '(s) - <span class="trinstrong1">' + stmsgname + '</span></strong>  <br> Created from <br><strong><span class="trinspam">' + dataarray['applyon'] + '(s) :</span></strong> ' + fromnamenew;
                                        }
                                    }
                                    else {
                                        if (returndata['action'].includes('Split') && returndata['action'].includes('Full Merge')) {
                                ...
                                if (dataarray['applyon'] == 'Sub-District' && returndata['clickpopup'] == 'submerge') {
    
    
                                    var myflag = '';
                                    var myflagpl = '';
                                    var fromnamenewpl = '';
                                    mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Partially Submerged ' + dataarray['applyon'] + '</th><th>Partially Submerged ' + come + '(s)</th></tr></thead><tbody>';
                                    ...
                                    for (var k = 0; k < namefromtext.length; k++) {
                                        ...
                                        fromnamenew += '<strong class="trinstrong1">' + namefromtext[k] + '</strong>, ';
                                        myflag = 'Submerged';
                                    ...
                                    for (var l = 0; l < partiallylevel.length; l++) {
                                        ...
                                        fromnamenewpl += '<strong class="trinstrong1">' + partiallylevel[l] + '</strong>, ';
    
    
                                        if (dataarray['addlinksDTIDtext' + l + ''] != '') {
                                            mainaction += '<tr class="trleftalign"><td><strong><span class="trinstrong1">' + partiallylevel[l] + '</span><span class="trinspam">(' + dataarray['applyon'] + ')</span></strong></td><td><strong class="trinstrong">' + partiallylevel[l] + ' - </strong>' + dataarray['addlinksDTIDtext' + l + ''].replaceAll(",", ", ") + '</td></tr>';
                                    ...
                                    if (totalutcount == 0 && totalstcount != 0) {
                                        fmsg = '(Total - ' + totalstcount + ' ) <strong><span>' + dataarray['applyon'] + '(s) - </span></strong>' + fromnamenew + ' <strong class="trinstrong1">(' + myflag + ')</strong>  <br> into <br> Sea / River / Any Other';
                                    }
                                    else if (totalstcount == 0 && totalutcount != 0) {
    
                                        fmsg = '(Total - ' + totalutcount + ' ) <strong><span>' + dataarray['applyon'] + '(s) - </span></strong>' + fromnamenewpl + ' <strong class="trinstrong1">(' + myflagpl + ')</strong> <br> Submerged into <br> Sea / River / Any Other';
    
                                    }
                                    else {
                                        fmsg = '(Total - ' + totalstcount + ' ) <strong><span>' + dataarray['applyon'] + '(s) - </span></strong>' + fromnamenew + ' <strong class="trinstrong1">(' + myflag + ')</strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong><span>' + dataarray['applyon'] + '(s) - </span></strong>' + fromnamenewpl + ' <strong class="trinstrong1">(' + myflagpl + ')</strong><br> into <br> Sea / River / Any Other';
                                    ...
                                    if (dataarray['applyon'] == 'State') {
                                        mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Merge / Partially Merge - State / UT </th><th>Assigned District(s)</th><th>Assigned Partially District(s)</th></tr></thead><tbody>';
                                    }
                                    else {
    
                                        mainaction += '<table class="table table-bordered mb-0 table-hover"><thead><tr class="trhead"><th> Merge / Partially Merge - ' + dataarray['applyon'] + '</th><th>Assigned ' + come + '(s)</th><th>Assigned Partially ' + come + '(s)</th></tr></thead><tbody>';
                                    }
                                    ...
                                    for (var k = 0; k < returndata['namefrom'].length; k++) {
                                        ...
                                        if (dataarray['applyon'] == 'State') {
                                            ...
                                            if (returndata['StateStatus'][0] == 'ST') {
                                                if (returndata['toStatus'][0] == returndata['StateStatus'][0]) {
                                                    toflag = '';
                                                }
                                                else {
                                                    toflag = '(Now State)';
                                            ...
                                            fromnamenew = '<strong class="trinstrong">' + returndata['nametotext'] + '<span class="trinspam">' + toflag + '</span></strong>, ';
    
                                            if (returndata['ostate'][k] == 'ST') {
                                                $statusstate = '<strong class="trinspam">State: </strong>'
                                            }
                                            else {
                                                $statusstate = '<strong class="trinspam">Union Territory: </strong>'
                                            }
                                            //modified by sahana to change Partially merge table summary 01-08-2023
    
                                            if (dataarray['addlinksDTIDtext' + k + ''] != '') {
                                                fromdatacoccatename += ' ' + $statusstate + '<strong class="trinstrong">' + namefromtext[k] + ' - </strong>' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '<br>';
                                            }
                                            if (dataarray['addpartiallyleveltext' + k + ''] != '') {
                                                fromdatacoccatenamep += ' ' + $statusstate + '<strong class="trinstrong">' + namefromtext[k] + ' - </strong>' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '<br>';
                                            }
                                            ...
                                            if (returndata['action'][k] == 'Merge') {
                                                ...
                                                if (returndata['ostate'][k] == 'ST') {
                                                    ...
                                                    utmsgname += namefromtext[k] + '<span class="trinspam">' + utflag + '</span> , '; //01-08-2023
                                                }
                                            }
                                            ...
                                            if (returndata['ostate'][k] == 'ST') {
                                                ...
                                                fromnamenew = '<strong><span class="trinspam">' + sssflag + '</span></strong> <strong class="trinstrong">' + returndata['nametotext'] + '</strong>, ';
    
                                                if (dataarray['addlinksDTIDtext' + k + ''] != '') { //01-08-2023
                                                    fromdatacoccatename += '<strong class="trinstrong">' + namefromtext[k] + '<span class="trinspam"> ' + sssflag + '</span>&nbsp</strong>' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '<br>';
                                                }
                                                if (dataarray['addpartiallyleveltext' + k + ''] != '') {//01-08-2023
                                                    fromdatacoccatenamep += '<strong class="trinstrong">' + namefromtext[k] + '<span class="trinspam"> ' + sssflag + '</span>&nbsp</strong>' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '<br>';
                                                }
    
                                            }
                                            //Defect_JC_92, modified by sahana for partially split and merge
                                            else if (returndata['action'][k] == 'Partially Split & Merge') {
                                                //modified by sahana to change UI for partially Split and merge in district level summary 1809
                                                fromnamenew = '<strong><span class="trinspam">' + sssflag + '</span></strong> <strong class="trinstrong">' + returndata['nametotext'] + '</strong>, ';
                                                if (dataarray['addlinksDTIDtext' + k + ''] != '') { //01-08-2023
                                                    fromdatacoccatename += '<strong><span class="trinspam"> ' + sssflag + '</span></strong><strong class="trinstrong">' + namefromtext[k] + '-&nbsp</strong>' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '<br>';
                                                }
                                                if (dataarray['addpartiallyleveltext' + k + ''] != '') {//01-08-2023
                                                    fromdatacoccatenamep += '<strong><span class="trinspam"> ' + sssflag + '</span></strong><strong class="trinstrong">' + namefromtext[k] + '-&nbsp</strong>' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '<br>';
                                                }
                                                ...
                                                fromnamenew = '<strong><span class="trinspam">' + utsssflag + '</span></strong> <strong class="trinstrong">' + returndata['nametotext'] + '</strong>, ';
    
                                                if (dataarray['addlinksDTIDtext' + k + ''] != '') {//01-08-2023
                                                    fromdatacoccatename += '<strong><span class="trinspam"> ' + utsssflag + '</span> </strong><strong class="trinstrong">' + namefromtext[k] + '</strong>-' + dataarray['addlinksDTIDtext' + k + ''].replaceAll(",", ", ") + '<br>';
                                                }
                                                if (dataarray['addpartiallyleveltext' + k + ''] != '') { //01-08-2023
                                                    fromdatacoccatenamep += '<strong><span class="trinspam"> ' + utsssflag + '</span> </strong><strong class="trinstrong">' + namefromtext[k] + '</strong>-' + dataarray['addpartiallyleveltext' + k + ''].replaceAll(",", ", ") + '<br>';
                                                }
                                            }
                                    ...
                                    if (dataarray['applyon'] == 'State') {
                                        ...
                                        mainaction += '<tr class="trleftalign"><td><strong>' + toStatus + ': <span class="trinstrong1">' + nameflag + '</span>&nbsp<span class="trinspam">' + toflag + '</span></strong></td><td>' + fromdatacoccatename.replaceAll(",", ", ") + '</td><td>' + fromdatacoccatenamep.replaceAll(",", ", ") + '</td></tr>';
                                    }
                                    else {
                                        //modified by sahana to change Partially merge table summary 01-08-2023
                                        mainaction += '<tr class="trleftalign"><td><strong><span class="trinspam">' + dataarray['applyon'] + ': </span><span class="trinstrong1">' + nameflag + '</span></strong></td><td>' + fromdatacoccatename.replaceAll(",", ", ") + '</td><td>' + fromdatacoccatenamep.replaceAll(",", ", ") + '</td></tr>';
                                    }
                                    ...
                                    if (dataarray['applyon'] == 'State') {
                                        ...
                                        var ji = '';
                                        if (stmsgname != '') //ST- merge/ partially merge
                                        {
                                            ji = '<span class="trinstrong1">' + stmsgname + '</span> ' + stflag + '';
                                        }
    
                                        var ji1 = '';
                                        if (utmsgname1 != '') //UT- merge/ partially merge
                                        {
                                            ji1 = '<span class="trinstrong1">' + utmsgname1 + '</span> ' + utflag + '';
                                        }
    
                                        var jji = '';
                                        if (utmsgname != '') //UT- merge/ partially merge
                                        {
                                            jji = '<span class="trinstrong1">' + utmsgname + '</span> ' + utflag + '';
                                        }
    
                                        var jji1 = '';
                                        if (stmsgname1 != '') //ST- merge/ partially merge
                                        {
                                            jji1 = '<span class="trinstrong1">' + stmsgname1 + '</span> ' + stflag + '';
                                        }
                                        ...
                                        if (totalutcount == 0 && totalstcount != 0) {
                                            fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> : ' + ji + ' ' + ji1 + ' </strong> <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong> ' + fromnamenew + '';
                                        }
                                        else if (totalstcount == 0 && totalutcount != 0) {
    
                                            fmsg = '(Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + ' ' + jji1 + '</strong> <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong> ' + fromnamenew + '';
    
                                        }
                                        else {  //modified by sahana to differentiate condition on Many to one for MP/PM //0509
                                            if (stmsgname && utmsgname && !utmsgname1 && !stmsgname1) {
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> : ' + ji + ' </strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + '</strong>  <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                            }
                                            else if (utmsgname1 && stmsgname1 && !stmsgname && !utmsgname) {
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> : ' + ji1 + ' </strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> :  ' + jji1 + '</strong>  <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                            }
                                            else if (stmsgname && !utmsgname1 && utmsgname && stmsgname1) {
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> : ' + ji + '</strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + ' &nbsp ' + jji1 + ' </strong>  <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                            }
                                            else if (!stmsgname && utmsgname1 && utmsgname && stmsgname1) {
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> : ' + ji1 + ' </strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + ' &nbsp ' + jji1 + '</strong>  <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                            }
                                            //modified by sahana 08-09-2023 
                                            else if (stmsgname && utmsgname1 && utmsgname && stmsgname1) {
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> : ' + ji + ' &nbsp ' + ji1 + ' </strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + ' &nbsp ' + jji1 + ' </strong>  <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                            }
                                            //modified by sahana 18-09-2023
                                            else if (stmsgname && utmsgname1 && utmsgname && !stmsgname1) {
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> :' + ji + ' &nbsp ' + ji1 + '</strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + '</strong>  <br> into2 <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                            }
                                            //modified by sahana 18-09-2023
                                            else if (stmsgname && utmsgname1 && utmsgname && !stmsgname1) {
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> :' + ji + ' &nbsp ' + ji1 + '</strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + '</strong>  <br> into2 <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                                ...
                                                fmsg = '(Total - ' + totalstcount + ' ) <strong> - <span>' + sssflag1 + '(s)</span> :' + jji1 + ' &nbsp ' + ji1 + ' </strong></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - <span>' + utsssflag1 + '(s)</span> : ' + jji + ' &nbsp ' + jji1 + '</strong>  <br> into <br><strong><span class="trinspam">' + toStatus + ' : </span></strong>' + fromnamenew + '';
                                            }
                                        }
                                    ...
                                    //Defect_JC_92, modified by sahana for partially split and merge
                                        ...
                                        fmsg = '(Total - ' + totalutcount + ' ) <strong> - ' + dataarray['applyon'] + '(s) : <span class="trinstrong1">' + utmsgname + '</span> (' + utflag + ')</strong>  <br> into <br> ' + fromnamenew + '';
                                    }
                                    else {
                                        // fmsg = '(Total - '+totalstcount+' ) <strong><span class="trinstrong1">'+stmsgname+'</span> '+dataarray['applyon']+'</strong>  <br> into <br> '+fromnamenew+'';
                                        if (totalutcount == 0 && totalstcount != 0) {
                                            fmsg = '(Total - ' + totalstcount + ' ) <strong> - ' + dataarray['applyon'] + '(s) : <span class="trinstrong1">' + stmsgname + '</span></strong> <br> into <br> ' + fromnamenew + '';
                                        }
                                        else if (totalstcount == 0 && totalutcount != 0) {
    
                                            fmsg = '(Total - ' + totalutcount + ' ) <strong> - ' + dataarray['applyon'] + '(s) : <span class="trinstrong1">' + utmsgname + '</span></strong>  <br> into <br> ' + fromnamenew + '';
    
                                        }
                                        else {
                                            fmsg = '(Total - ' + totalstcount + ' ) <strong> - ' + dataarray['applyon'] + '(s) : <span class="trinstrong1">' + stmsgname + '</span></strong><br> AND <br> (Total - ' + totalutcount + ' ) <strong> - ' + dataarray['applyon'] + '(s) : <span class="trinstrong1">' + utmsgname + '</span></strong>  <br> into <br> ' + fromnamenew + '';
    ...
    $("#olddocumentselect").submit(function (e) {
        ...
        var rows_selected = $('#alreadydoc-datatable').DataTable().column(0).checkboxes.selected();

        if (rows_selected.length == 0) {

            Command: toastr["error"]("Please select at least one record to continue.");
            return false;
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (res[0] == 'adddata') {
                            ...
                            toastr.info("The Document you are trying to upload may already Exist. Please Verify...");
                            // enableSubmitButton();
                            return false;
                        } else if (res[0] == 'error') {
                            // veena toaster msg
                            toastr.error("This Document number already exists. Please use a unique number");
                            // enableSubmitButton();
                            return false;
                        }
                    },
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 'addeduser') {
                    ...
                    toastr["error"]("This Mobile Number already exists. Use another to process further.");
                    return false;
                }
                else if (data == 'userlimitexceeded') {
                    toastr["error"]("ORGI User Limit Exceeded, Can be upto 15 Users only.");
                    return false;
                }
                else if (data == 'dcouserlimitexceeded') {
                    toastr["error"]("DCO User Limit Exceeded, Can be upto 30 Users only.");
                    return false;
                }
                else if (data == 'adminnameexists') {
                    toastr["error"]("DCO Admin Login ID already exists. Cannot have more than 1 Admin.");
                    return false;
                }
    ...
    $('#users-units-datatable tbody').on('click', 'tr td.btnresetpass', function (event) {
        ...
        $.ajax({
            ...
            success: function (data)   // A function to be called if request succeeds
            {
                ...
                if (data == 'changepassword') {
                    ...
                    $('#resetcon-close-modal').find('form select').select2().trigger('change');
    ...
    var userstable = $("#users-units-datatable").DataTable({ "order": [[0, "desc"]], "scrollX": "100%", "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });
    ...
    var circularstable = $("#circulars-units-datatable").DataTable({ "order": [[0, "desc"]], "scrollX": "100%", "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });

    var documents = $("#documents-units-datatable").DataTable({ "order": [[0, "desc"]], "scrollX": "100%", "fixedHeader": true, "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });



    var table = $("#units-datatable").DataTable({ "order": [], "scrollX": "100%", "fixedHeader": true, "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });

    var tablestate = $("#tST-datatable").DataTable({ "order": [], "scrollX": "100%", "fixedHeader": true, "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });
    var tabledist = $("#tDT-datatable").DataTable({ "order": [], "scrollX": "100%", "fixedHeader": true, "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });
    var tablesdist = $("#tSD-datatable").DataTable({ "order": [], "scrollX": "100%", "fixedHeader": true, "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });
    ...
    var tableindex = $("#index-datatable").DataTable({ "order": [], "scrollX": "100%", "fixedHeader": true, "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });
    ...
    var tablelink = $("#unitslink-datatable").DataTable({ "order": [], "scrollX": "100%", "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]] });



    $('#units-datatable tbody').on('click', 'tr td:last-child a.dropdown-toggle', function () {
        $("a.dropdown-toggle").dropdown();
        return false;
    });


    $('#units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnEditnew', function (event) {
    ...
    $('#districts-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnlink', function (event) {

        return false;
    });

    $('#users-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.deletetablerow', function (event) {
                ...
                $.ajax({
                    ...
                    success: function (data)   // A function to be called if request succeeds
                    {
                        ...
                        if (data == 'deletedata') {
                            ...
                            Swal.fire({ title: "Deleted", text: "Your record has been deleted.", type: "success" }).then(function (t) { location.reload(); });
    ...
    $('#units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.deletetablerow', function (event) {
    ...
    $('#units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.noaction', function (event) {
        Swal.fire({
    ...
    $('#districts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    $('#districts-units-datatable tbody').on('click', 'tr td a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    $('#subdistricts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    $('#subdistricts-units-datatable tbody').on('click', 'tr td a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    $('#villages-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    $('#villages-units-datatable tbody').on('click', 'tr td a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    $('#wards-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    $('#wards-units-datatable tbody').on('click', 'tr td a.noaction', function (event) {
        Swal.fire({
            title: "Not authorized to access",
            text: "You are not authorized to access",
            type: "warning",
    ...
    var table1 = $('#districts-units-datatable').DataTable({
        "ordering": false, "scrollX": "100%", "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]]
    });


    $('#districts-units-datatable tbody').on('click', 'tr td:last-child a.dropdown-toggle', function () {
        $("a.dropdown-toggle").dropdown();
        return false;
    });

    $('#districts-units-datatable tbody').on('click', 'tr td span', function (event) {
        var data = JSON.parse(event.target.dataset.todo);
        window.location.href = 'adddocument?ids=' + btoa(event.target.dataset.todo);
        return false;
    });
    ...
    $('#subdistricts-units-datatable tbody').on('click', 'tr td span', function (event) {
        var data = JSON.parse(event.target.dataset.todo);
        window.location.href = 'adddocument?ids=' + btoa(event.target.dataset.todo);
        return false;
    });

    $('#villages-units-datatable tbody').on('click', 'tr td span', function (event) {
        var data = JSON.parse(event.target.dataset.todo);
        window.location.href = 'adddocument?ids=' + btoa(event.target.dataset.todo);
        return false;
    });
    ...
    $('#users-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnEditnew', function (event) {
    ...
    $('#districts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnEditnew', function (event) {
    ...
    $('#districts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.deletetablerow', function (event) {
    ...
    var subdistrictstable = $('#subdistricts-units-datatable').DataTable({
        "order": [[0, "asc"]], "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]]
    });



    $('#subdistricts-units-datatable tbody').on('click', 'tr td:last-child a.dropdown-toggle', function () {
        $("a.dropdown-toggle").dropdown();
        return false;
    });
    ...
    $('#subdistricts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnEditnew', function (event) {
    ...
    $('#subdistricts-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.deletetablerow', function (event) {
    ...
    var villagestable = $('#villages-units-datatable').DataTable({
        "order": [[3, "asc"]], "pageLength": -1, "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]]
    });

    $('#villages-units-datatable tbody').on('click', 'tr td:last-child a.dropdown-toggle', function () {
        $("a.dropdown-toggle").dropdown();
        return false;
    });


    $('#villages-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnEditnew', function (event) {
    ...
    $('#villages-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.deletetablerow', function (event) {
    ...
    $('#villages-units-datatable tbody').on('click', 'tr', function () {
        ...
        if (rowdata[8] == 'TOWN') {
            window.location.href = 'wards?ids=' + btoa('321**' + ids + '**123**' + atob($('#ids').val()));
        }


    });

    $('#documents-units-datatable tbody').on('click', 'tr td:last-child a.dropdown-toggle', function () {
        $("a.dropdown-toggle").dropdown();
        return false;
    });


    $('#documents-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.documentlinklink', function (event) {
    ...
    $('#documents-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnEditnew', function (event) {
        ...
        $("#upploaded_doc").attr("href", "Alldocuments/" + data.docstid + "/" + data.docnotification + "");
    ...
    $('#documents-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.deletetablerow', function (event) {
    ...
    var wardstable = $('#wards-units-datatable').DataTable({
        "order": [[3, "asc"]], "lengthMenu": [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]]
    });


    $('#wards-units-datatable tbody').on('click', 'tr td:last-child a.dropdown-toggle', function () {
        $("a.dropdown-toggle").dropdown();
        return false;
    });


    $('#wards-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.btnEditnew', function (event) {
    ...
    $('#wards-units-datatable tbody').on('click', 'tr td .dropdown ul.dropdown-menu a.deletetablerow', function (event) {
