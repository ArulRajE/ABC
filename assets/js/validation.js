...
function get_fromvalue1(value, i) {
            ...
            if (finalresult[0] == "getdata") {
                ...
                if (i != "") {
                    ...
                    $("#fstatus" + finalresult[2] + " option[value=" + kl + "]").prop(
                        "disabled",
                        false
                    ); // to disable freeze changed by srikanth jc-44
                    $("#fstatus" + finalresult[2] + "")
                        .val(finalresult[3])
                        .trigger("change");
                }
    ...
    // }
        ...
        var statenew = $("#todataaction_1")
            .find('select[name^="statenew"] option:selected')
            .map(function () {
                if (this.value != "") {
                    return this.value;
                }
    ...
    // JC_11 Arul
        ...
        var action = $("#row_" + rn)
            .find('select[name^="action"] option:selected')
            .map(function () {
                if (this.value != "") {
                    return this.value;
                }
...
function get_to_data(value, i) {
    // JC_11 Arul
    var rn = $("#rowno").val();
    var namefrom = $("#row_" + rn)
        .find('select[name^="namefrom"] option:selected')
        .map(function () {
            if (this.value != "") {
                return this.value;
            }
    ...
    if (value.value != "") {
        ...
        if (
            value.value != "" &&
            $(value).siblings("ul.parsley-errors-list").length > 0
        ) {
    ...
    //jc-35 done by bheema
        ...
        value.value == "" &&
        ($("#clickpopup").val() == "Rename" || $("#clickpopup").val() == "Merge") &&
        $("#comefromcheck").val() == "State"
    ) {
            ...
            if (
                ...
                if (mul[1] == "undefined" || mul[1] == undefined) {
                    ...
                    if ($("#partiallyids").val() != "") {
                        ...
                        $("#action1")
                            .val(jsonresult[0]["forreaddetails"])
                            .trigger("change");
                    }
                    // Ends...
                    // // Reset Sub district
            ...
            if (seleted != "State") {
                if (
                    ...
                    if (mul[1] == "undefined" || mul[1] == undefined) {
                        ...
                        if (
                            $("#sddistrictget_1")
                                .siblings("ul.parsley-errors-list")
                                .find("li").length > 0
                        ) {
...
function get_district_popupto_ii(data, clickpopup, i) {
    ...
    //srikanth to refresh village list
        ...
        (clickpopup == "Merge" || clickpopup == "Rename") &&
        data.value == "" &&
        seleted == "Village / Town"
    ) {
        $("#districtnew" + i + "")
            ...
            .trigger("change");
        {
            ...
            $("#newnamecheck" + i + "")
                .val("")
                .trigger("change"); //new name refresh changed by srikanth
        }
    }
    //ends here
    else {
...
function get_district_popupto(data, clickpopup, i) {
    ...
    if (
        (clickpopup === "Merge" || clickpopup === "Reshuffle") &&
        data.value !== "" &&
        (seleted === "District" ||
            seleted === "Village / Town" ||
            seleted === "Sub-District") &&
        namefrom.length > 0 &&
        action.length > 0
    ) {
        DisableRow(rn);
...
function get_sddistrict_popupto(data, clickpopup) {
    ...
    //sub district dropdown reset done by bheema
        ...
        data.value == "" &&
        (clickpopup == "Rename" || clickpopup == "Merge") &&
        seleted == "Sub-District"
    ) {
        $("#named2021").val("").trigger("change");
        $("#newnamecheck").val("").trigger("change");
...
$("#name2021").keyup(function () {
    ...
    var districtnew = $("#todataaction_1")
        .find('select[name^="districtnew"] option:selected')
        .map(function () {
            if (this.value != "") {
                return this.value;
            }
    ...
    var sddistrictnew = $("#todataaction_1")
        .find('select[name^="sddistrictnew"] option:selected')
        .map(function () {
            if (this.value != "") {
                return this.value;
            }
    ...
    if (come == "Sub-District") {
        if (
            ...
            clickpopup == "Addition" &&
            sddistrictnew.length == 1 &&
            value.length > 0
        ) {
            EnableAddButton2();
        } else if (
            clickpopup == "Addition" &&
            sddistrictnew.length == 0 &&
            value.length == 0
        ) {
            DisableAddButton2();
        }
    ...
    $("#adddocumentnext").submit(function (e) {
        ...
        if (tempflag != true) {
            ...
            if ((tempflag == true && addlinksDTIDLength == 1) || namefromtext.length == 1) {
                ...
                if (
                    confirm(
                        "You are processing this Action without choosing the records from all the blocks. Would you like to continue?"
                    ) == true
                ) {
                    adddocumentnextsubmit();
                } else {
                            ...
                            if (
                                ...
                                for (var k = 0; k < returndata["namefrom"].length; k++) {
                                    if (dataarray["applyon"] == "State") {
                                        ...
                                        if (returndata["StateStatus"][0] == "ST") {
                                            if (
                                                returndata["toStatus"][0] ==
                                                returndata["StateStatus"][0]
                                            ) {
                                                toflag = "";
                                            } else {
                                                toflag = "(Now State)";
                                        ...
                                        if (returndata["ostate"][k] == "ST") {
                                            $statusstate =
                                                '<strong class="trinspam">State: </strong>';
                                        } else {
                                            $statusstate =
        ...
        if (rowdata[7] != "") {
            ...
            try {
                ...
                if (response.ids) {
                    ...
                    window.location.href =
                        "districts?ids=" + btoa("321**" + ids + "**123");
...
function EnableRow(rn) {
    if (rn == 1) {
        ...
        $("#ms-id2021" + rn + " [class*=ms-elem-selectable]").removeClass(
            "disabled"
        );
        $("#ms-id2021" + rn + " [class*=ms-elem-selection]").removeClass(
            "disabled"
        );
        $("#row_" + rn + "")
            .find("input, textarea, button, select")
