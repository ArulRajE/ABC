...
class SSP {
     ...
     */
    static function simple ( $request, $pg_details, $table, $primaryKey, $columns, $filtroAdd=NULL, $other=null)
    {
        ...
        $resTotalLength = pg_query( $db,"SELECT count(*) OVER() AS full_count1 FROM $table $where" );
        $recordsTotalRow = pg_fetch_all($resTotalLength);

        $recordsTotal = $recordsTotalRow[0]['full_count1'];
        pg_free_result( $result );
        pg_free_result( $resTotalLength );
        $otherdata ='';
       //  print_r($other);
        if($other['stids']!='' && $other['dtids']=='')
        {
           // echo "asaassss";
             $other = pg_query( $db,"SELECT \"DTID\",\"DTName\" FROM dt2021 where \"STID\"=".$other['stids']." AND \"is_deleted\"=1" );
             $otherdata = pg_fetch_all($other);
        }
       else if(isset($other['stids']) && $other['stids']!='' && $other['dtids']!='')
        {
             $other1 = pg_query( $db,"SELECT \"SDID\",\"SDName\" FROM sd2021 where \"STID\"=".$other['stids']." AND \"DTID\"=".$other['dtids']." AND \"is_deleted\"=1" );
             $otherdata = pg_fetch_all($other1);
        }
      //  print_r($otherdata);
     ...
     static function simplenew ( $request, $pg_details, $table, $primaryKey, $columns, $filtroAdd=NULL )
    {
        $db = SSP::pg_connect( $pg_details );
   ...
   $select = "SELECT ".implode(", ", SSP::pluckas($columns))."
    ...
    static function pg_connect( $pg_details )
    {
        $db =  pg_connect("
            ...
            password={$pg_details['pass']} options='--client_encoding=UTF8'") or SSP::fatal("Error en conexión a DB.\n". pg_last_error()) ;
        return $db;
    }
